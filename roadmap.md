...
- **Layout**: no page heading (the tab nav names it — applies to every tab now); the playback-volume knob shares the top row with the Fixed-volume card (a `card-grid` pair), the Range bar spans full-width below, and Automatic is a full-width card (its two checkboxes packed two-up). Same card style as the Output backend sections. Optimal ISO sits at the top of the Fixed-volume card as an independent fixed-volume mode (not gated by the enable checkbox — see the field-mapping correction below); only the dBFS level is indented under the Fixed-volume enable checkbox.
- **Field mapping** (live `/config` + readme): `volume_fixed` is **Optimal ISO** (inter-sample-overs-optimized fixed volume), `fixed_volume` is the dBFS level, `fixed_volume_enabled` gates the dBFS level. The level grays when fixed-volume is off **or** Optimal ISO is on (ISO supersedes the manual level). Only `adaptive_volume` is live; the rest are http/restart. **Correction (2026-07-21):** the earlier "`fixed_volume_enabled` gates both" wording was wrong — `volume_fixed` (Optimal ISO) is an **independent** fixed-volume mode with its own 0/−3/−6 dB enable (readme attr `volume_fixed`), not gated by `fixed_volume_enabled`. Graying Optimal ISO when fixed-volume was off trapped a nonzero `volume_fixed` the user could no longer clear (locked the live volume control). Optimal ISO is now gated only by Direct SDM; see CHANGELOG `### Fixed`.
...
Corollary — the "loudness comes back disabled at defaults" symptom is **not** matrix gating. The Speakers snapshot carries `<plugin type="loudness" enabled="0" low_level="20">` — literally disabled-at-defaults — and (originally) every apply rebuilt the working config from `snapshot ⊕ staged edits`. So a post-process setting survived only the apply that carried it; the next unrelated apply re-asserted the snapshot and reverted it. **Resolved 2026-07-20 (see the apply-lane entry below): apply now rebuilds from the running config, so a prior apply's edits survive.** The open Phase 5 question — whether apply should preserve current state rather than re-asserting the snapshot's — is answered: it preserves.
...
Apply-lane rewrite + two bug fixes (2026-07-20):

- **Apply no longer reverts settings it isn't currently staging.** `presetconf.restore_zip_from_snapshot` → `restore_zip_from_running`: the `POST /restore` working config is now `{running config} ⊕ {edits}`, built from the backup's `hqplayerd.xml`, not `{active snapshot} ⊕ {edits}`. The old snapshot rebuild reset every field the user hadn't staged in *that* apply back to the preset's stored value, so two sequential applies clobbered each other (stage `direct_sdm` → `volume_fixed` reverts; stage `volume_fixed` → `direct_sdm` reverts). **Reproduced live on 6.0.4** by driving the REST API: applied `direct_sdm=0`+save, then `volume_fixed=2`+save, and watched `direct_sdm` return to `1` from `data/cfgs/Speakers.xml`. Tradeoff taken deliberately: discarding daemon-side drift is not worth discarding the user's own prior edits. This is what resolves the loudness/post-process "reverts on next apply" corollary above.
- **`_backup_or_cached(for_write=True)` refuses the stale-archive fallback.** The empty-`/backup` workaround (serve the last healthy archive when the daemon returns a broken zip after a profile load/save) is correct for *reads* but not *writes*: a restore built on a stale archive silently resurrects whatever changed since it was cached. Writes now raise instead of writing old values back.
- **Preset preview no longer lights Apply for a no-op.** Selecting the already-active preset used to set `pendingPreset`, lighting Apply; pressing it fired a destructive `switch_to` reload of the loaded preset, which trips the daemon's empty-`/backup` window so the apply never confirms and Apply stays stuck. Fix (`state.js`): previewing the active preset clears the preview instead of pending a switch, and `applyAll` drops a `switch_to` equal to the active preset. **Reproduced and re-verified live** (headless browser on a second instance, port 8091): Speakers → Office (pends) → Speakers (clears, Apply dark). Root cause confirmed live via API repro before the fix.
- **`hqptuner/httplane.py`** split out of `manager` (the persistent write lane: restore + verify + retry + unfixable-device classification), forced by the 500-line gate after the apply fix and warranted on merit — mirrors `enginelane.py`. `manager` exposes a small accessor surface for it rather than the lane reaching into privates.
- **Volume tab** — three-handle Range control (Min/Startup/Max on one shared −120…+12 dBFS axis, distinct thumbs, priority-suppressed tick labels, drag bubble) replacing three number boxes; large-knob rendering fixes (visible full-sweep track, dial ticks, readout tied under the dial, uniform disabled dim, notice pill); header/help-text/card-alignment consistency. Verified against the headless render, not asserted.
- **Smaller UI**: source chip reads `N/A` when idle (was echoing the configured output mode); apodizing-only filter narrowing defaults on; FFT filter-length card auto-collapses unless an FFT filter is selected (matched by option name, not the volatile index); redundant leading names stripped from the `sdm_integrator`/`sdm_conversion` option prose; Mode/Backend/rate segments laid out horizontally.
- **No regression test.** The three fixes above are frontend JS or a lane whose durable guard would need to move to the backend to be gate-testable (the repo has no JS runner); design was written up but the test was deferred by decision.

- **Preset system rewritten — HQPTuner now owns presets (2026-07-20).** hqplayerd's `profile/save` to an *existing* name is a silent no-op (HTTP 200, snapshot never updated — **reproduced live** by REST-driving a save cycle and reading `data/cfgs/Speakers.xml`), so "Apply & Save" never persisted to a named preset — the reported "save doesn't take / profiles get confused" bug. Root-caused further: `POST /restore` is `[default]`-centric (lands the daemon on `hqplayerd.xml`, discards a named working member) and additive (writes members but can't delete). Rather than keep special-casing the daemon's broken profile subsystem, HQPTuner now keeps presets as full-config XML in a store it owns (`presetstore.py`, `HQPTUNER_PRESET_DIR`, gitignored user data) and drives the daemon through the one reliable primitive — `POST /restore` onto `[default]` — mirroring each preset into `data/cfgs` so the native UI stays populated (`profile/delete` for removals). Load/save/delete rewired (`manager`), migration imports existing daemon presets on first connect, ephemeral Apply stays ephemeral by design, and a Delete button lands on the preset picker. **This reverses the Phase 5 "no custom snapshot store" decision below** — that decision assumed the daemon's profile CRUD worked; it does not. Full findings: `docs/protocol.md` §3.6. **Validated live 2026-07-20:** save → load-away → load-back preserves the change; back-to-back loads survive the restart window. New offline tests: `test_presetstore.py`, `test_preset_lifecycle.py`.

## Phase 4M — Matrix pipeline editing (approved 2026-07-20; **complete 2026-07-20**)

**Status: all seven delivery phases hand-back-PASSED and committed** (probes `316a032`, read lane `2e52b9d`, editing+apply `46f86c3`, stage editor+upload `fb28e8b`, profiles `8b4cf3f`, import `225f4a2`, plots `1f242c7`). Per-phase findings, spec adjustments (Lin gain shipped, plain profile-Save cut as a daemon silent no-op, client-FFT plot ruling, numeric acceptance amendment), and post-delivery follow-ups (design pass, switch-while-playing) live in `docs/matrix-spec.md`'s delivery checklist.

Matrix editing is **un-cut** (reverses outline §1's non-goal; user decision). Design of record: `docs/matrix-spec.md` — dedicated Matrix tab (between Volume and System), pipeline rows as signal-flow chips, inline stage editor with two-way raw-string sync, AutoEq/REW txt import, client-side magnitude+phase response plots (dsp.js extended with the full RBJ set), matrix-profile CRUD via the daemon's `/matrix/{load,save,delete}` routes plus live switch via 4321 `MatrixSetProfile`. Post-process controls stay on the DSP tab.
...
- **Export EQ** (`features.md` #7): the write-side twin of the AutoEq/REW importer. `hqptuner/static/lib/eqexport.js` serializes matrix pipeline `iir` stages back to REW / Equalizer APO `.txt` — a per-row **Export EQ** button and a master **Export AutoEq / REW .txt…** for the whole pipeline set (stereo-identical collapses to one block; divergent channels get `# Pipeline N` section headers). Format survey + mapping in `docs/eq-export.md`; REW `.txt` only for v1.
