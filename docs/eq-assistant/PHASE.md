...
* **Midrange: about 3× under the most sensitive published figure.** The HD 650 chain varies its group delay by **156 µs** across 300 Hz–1 kHz. The lowest threshold in Liski's Table III is −0.45 ms (pink impulse, 500 Hz), and the lowest anywhere in §3.2 is 0.9 ms. Against real music signals (castanet, hi-hat: 1.4–4.6 ms) the margin is ten- to twenty-fold.
...
* **The exception is the bass, and it is unmeasured territory.** The chain reaches **4.06 ms at 37 Hz**. No published threshold exists below 500 Hz. The trend runs in our favour — thresholds rise steeply as frequency falls, 4.7 ms already at 250 Hz (Jensen & Møller) — but extending that to 37 Hz is **extrapolation, and must be labelled as such rather than asserted.**
...
That paragraph is the entire treatment of phase in the thesis; phase does not appear in his Conclusions at all.
...
* **No group-delay threshold below 500 Hz.** Which is precisely where our chain's delay lives (§2.4). The single highest-value gap in this document.
...
* **Moulana's Section 9 figures** carry two Q-label rows (3, 6, 12, 25, 50, 100, 200 above; 2, 4, 9, 18, 35, 75, 150 below) on apparently the same gridlines, with no explanation in the text. All values in §6 are read against the upper row. Resolve before digitising any of those curves.
