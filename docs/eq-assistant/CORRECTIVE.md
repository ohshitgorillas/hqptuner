...
1. **Same-rig-family reference curve.** A 5128 measurement gets a 5128-referenced target (`5128-df-target.txt` exists in this directory for exactly that); a 711-family measurement gets a 711-family target. **Harman is 711-family-referenced — do not translate it onto 5128 data** `[session]`. The rig-trust material behind this is `TRANSDUCERS.md` §4.
2. **Flat-on-this-rig, backed by cited testimony.** Legal when the rig is nonstandard but its behavior is attested, with any un-attested range held at reduced confidence until a same-rig reference file supplies that range's transfer. Confidence tracks evidence: the session ledger's `yardstick.pending_upgrade` field records exactly which magnitudes are provisional and why — name what you are waiting on rather than pretending the target is settled.
...
**Tilt the target; do not fit raw DF.** A raw diffuse-field curve is an anatomy transfer, not a preference target — Olive 2025 found **no listener cluster accepted the raw 5128 DF target**, and the mean preferred tilt is −1 dB/octave (`SOURCES.md` §2.2d). The −1 dB/oct figure is a convention with spread around it (`SOURCES.md` head note), so it is a starting point the user adjusts, not a constant.

**Deviate from the target deliberately and file the deviation.** A target is not a contract; a documented deviation (e.g. on preference-research grounds) is legitimate, but it is recorded in the turn record as a filed deviation. A later complaint in that region is resolved by re-reading the filed deviation, not by re-measuring. An undocumented deviation is indistinguishable from an error two turns later.
...
- **Free the centers in the refine pass.** Grid search places, continuous refinement converges; a band whose refined center lands off the measured feature center at finer smoothing gets moved.
