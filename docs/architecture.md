...
- **Live vs restart split.** Every control classified live or restart-required; pending-changes bar reports split before Apply. Tagging: `docs/settings-classification.md`. Split is **all-or-nothing per staged batch**: one restart-required field sends the whole batch down the restore lane (`lanes/livemap.split_live`) — the restart happens anyway, and a value applied live never reaches the config file the restart boots from, so a split batch would revert its own live half. Chain fields a staged apply does route live are recorded per chain (`lanes/livelane.remember_routed`), same record LIVE keeps, so auto-save and chain re-entry report and re-assert them while the chain is dormant.
...
- **No engine read may cross a restart boundary unproven.** `POST /restore` returns 200 when the archive reaches disk; the daemon restarts after that, and the departing process keeps serving both lanes for seconds. So neither the 200 nor `await_http_ready` is evidence a restart happened, and any read taken on that evidence answers for the process being replaced. The proof is a new Control API connection — `lanes/settle.await_restart`, off the connect generation — and both the engine re-read that refills live state (`settle.resync_engine_state`, whose readings every save and auto-save fold into a preset) and the config lane's readback verify (`httplane`) wait for it. A boundary that never arrives is never assumed: live state stays invalid and the apply reports `unrestarted` rather than confirming itself against its own upload.
