...
**Live presets are separate store, and deliberately so** (`hqptuner/presets/store/live.py`, routes in `api/routes/livepreset.py`). Config preset above is whole `hqplayerd.xml` applied by restarting daemon; live preset is handful of enum IDs applied through LIVE lane, so it never writes config file and never restarts anything. Daemon never sees them — one JSON file HQPTuner owns, schema-stamped on `store/presets.py` pattern (refuse newer stamp, adopt unstamped on next write), same name rule.
