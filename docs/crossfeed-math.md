...
Brown & Duda work in **interaural-polar coordinates**: θ measured from interaural axis, not median plane. θ = 0° points at ear in question. Source at azimuth ±30° from front center sits at θ = 60° for near ear, θ = 120° for far ear.
...
ITD is `delay:t=<seconds>` stage. Manual §7.2 gives plugin arguments as `s` (samples **at source rate**), `t` (seconds), `d` (meters), `v` (velocity, default 343.956 m/s) — so `t` rate-independent as *specification*, but underlying primitive is sample-granular at source rate. Resolution therefore **22.7 µs at 44.1 kHz**, ~4 % of 261 µs ITD, improving directly with rate.
...
`k = 0.5` puts center at unity gain at DC. Note `(1−α_n)` is **negative** (−0.3436) — near-ear row 2 phase-inverted, which makes near ear's mild HF *boost* come out right. Exactly the Lin-negative capability compensation block already relies on.
...
"Literal loudspeaker simulation" versus "flat center by construction" is not fork — endpoints of one continuous parameter. Neither is "structural" versus "HRTF" — two ways of supplying same two transfer functions into same row structure. One design follows.

### 6.1 · Topology

Hold side path at physical value, put only **center** on control:

```
G_S      = (H_n − H_f)/2                    side path — fixed, never moves
G_M(λ)   = λ·(H_n + H_f)/2 + (1 − λ)        center path — λ = 1 literal, λ = 0 flat
```
...
**λ = 1** zeroes rows 3–6, collapses to literal 8-row structure of §5 — full ±30° simulation, with its −1.80 dB center tilt and phantom-center comb.

**That comb is deep, and measured.** Over 20 Hz–20 kHz center response at λ=1, 30° has **11.59 dB dip at 1426 Hz** — straight through vocal presence. Single largest coloration design produces, order of magnitude above tilt everything else here discusses. Reasoning about depth from two paths' HF level difference (13.6 dB) gives wrong answer: notch sits where shadow filter has barely begun to act and two paths still comparable.

**λ = 0** gives center of exactly 1 at *every* frequency — delay term cancels between rows 3 and 7 identically, so no tilt and no comb, side path still fully processed. Verified numerically: at λ = 0 row coefficients sum to A + B = 1.000 at DC and at HF alike.

**Between them**, partial tilt and partial comb depth. λ is existing compensation slider, generalized: `s = 1 − λ` maps onto today's 0–150 % control with same sense (0 % = untouched crossfeed character, 100 % = neutral center, above 100 % = overshoot brighter).

Cost of generality: 16 rows against 8. Matrix allows 128, engine's `pipelines` setting has 16 option, so free in practice.

### 6.2 · The center control is a comb control

Measured, most useful thing to know about λ. Center response's peak-to-trough ripple against λ, three angles:
...
**Relationship essentially angle-independent**: ripple falls below 3 dB at λ ≈ 39 % for every angle tested (39/39/38), below 6 dB at λ ≈ 67 % (68/67/65). So control behaves identically wherever angle sits — move geometry and center setting needs no re-tuning to hold same artifact amount.
...
Note HF tilt runs *other* way — −2.08 dB at 22°, −1.80 at 30°, −1.14 at 45° — so any reasoning treating "wider needs more center correction" as tilt story has sign backwards.
...
| **λ** center character | literal loudspeaker center (λ=1) ↔ flat center (λ=0), side path untouched throughout | 0–150 % as `s = 1 − λ` |
...
Angle and center only. **Head size excluded and persists across preset changes** — anatomy rather than taste, and the one parameter with physically correct per-person answer, setting both `lp1` corner and ITD scale. Center values taken from ripple table above, not chosen by feel.

| Preset | Angle | Center | Center ripple | Notch | Rationale |
|---|---|---|---|---|---|
...
| **Neutral center** | 30° | 0 % | 0.00 dB | — | zero center coloration; nothing in hardware ships this |

Any manual touch of angle or center falls to **Custom**, derived rather than stored, matching Bauer preset dropdown's convention.
...
**Modeled** — `H = α + (1−α)·lp1`, two rows each, α and τ derived from θ and a. Exact, negligible CPU, and θ continuously sweepable because α and τ closed-form in it.
...
- Dataset licensing (all secondhand, verify before use): MIT KEMAR — free with citation; CIPIC — public-domain subset, redistribution permitted; HUTUBS — CC BY; ARI — CC BY-SA (viral, problematic for vendoring); 3D3A — CC BY; SADIE II — license not established.
...
| center tilt | fixed by preset, corrected by fitted 8-row block | λ, exact at every value |
...
**What removal does not reproduce.** Exactly two rows come back, one per ear — head that was three rows before install is not recoverable, and neither are rows compiler *set aside*, which install does not carry into block. That is install's behavior, not removal's: removal reads block in front of it and cannot know how block came to be.
...
- Vickers, E., "Fixing the Phantom Center: Diffusing Acoustical Crosstalk" — [PDF](https://www.sfxmachine.com/docs/FixingThePhantomCenter.pdf). Phantom-center comb and its ~2 kHz notch.
