...
**Rate per-family and friendly.** Persistent rate live in two fields — `defaults_samplerate` (PCM) and `defaults_bitrate` (SDM) — each target/ceiling. HQPTuner show both as fixed friendly menus (`1x…32x` / `DSD64…DSD2048`) mapped to **48k-base** ceiling value.

**Forced on every write (HQPTuner policy).** `auto_family=1`, `samplerate=0`, `bitrate=0` merged into every `/restore` payload (`FORCED_CONFIG`, `lanes/httplane.py`), so friendly per-family ceiling hold (auto-family follow source's 44.1/48 base; fixed sample/bit rate stay on Auto). Not exposed in UI. Enforced on write only.
