...
**Two rate slots per family, and they differ.** Daemon's own `/config` form label them apart: `defaults_samplerate` / `defaults_bitrate` = "Rate limit" (no Auto entry), `samplerate` / `bitrate` = "Sample rate" / "Bit rate" (Auto = `0`, and the slot `SetRate` writes). Limit = cap the engine follows in the source's base family. Request = the tier pinned; with `auto_family=1` (always forced) the engine keeps 44.1k material in its own family under the pin (owner-observed on the listening system, 2026-09-01; the 2026-07-28 `probe_rate_slots.py` reading of an exact 48k-base override is superseded by that).
...
**LIVE write the request slot as the tier.** `SetRate` is the only live rate setter, so LIVE carry it, always as the tier's 48k member (`chain.tier_rate`, both from the menu and from a stored preset rate) — falling back to the tier's other member when engine's rates list hold only that one, since device doing DSD in one base family only enumerate one member of every DSD tier and tier is reachable through either. Same rule govern graying: tier count as offered when list hold EITHER member. No config write is involved: a rate change never restarts the daemon. Reverse direction: `overrides.live_overrides` bring the pin back as its tier in the limit field, so tab agree after LIVE switch off, Apply stay unlit, Save persist it.
