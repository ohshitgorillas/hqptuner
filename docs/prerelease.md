...
`isDirty` (`hqptuner/static/store/resolve.js:178`) now compares through `truthy()` for `bool` schema entries as well as `widget: "checkbox"`, so a gate returned to its baseline reads clean and `stagedCount` drops back to zero. Covered by 48 tests in `tests/js/card-gates.test.js` (8 behaviors x 6 gates); 18 of them bite against the pre-fix code. No changelog entry — the gates are unreleased, so the latch never shipped.

Original report:

`isDirty` normalizes booleans only for `widget: "checkbox"`. The six enable gates converted to `widget: "segment"` with `bool: true` (loudness, crossfeed, DAC correction, matrix, log, fixed volume) compare their staged string against a boolean baseline: when the setting is enabled in the daemon, the baseline is JSON `true` (`httpconf.py:45`, `el.has_attr("checked")`), and a BYPASS→ENGAGE round-trip stages `"1"`, so `String("1") !== String(true)` keeps the gate dirty forever. `stagedCount` never returns to zero, the pending bar demands a no-op Apply on the restart lane, and only Discard clears it. None of the six schema entries carries `fileTruth`, so `formValue()` never coerces. `card-gates.test.js` asserts only the active segment, never the dirty round-trip.
