...
**Typography, surfaces, corner radius, motion and spacing are tokens-only (binding).** Never write a literal `font-size`, `font-weight`, `letter-spacing`, `border-radius`, `transition`, `gap`, `margin` or `padding` in `static/css/`, never a raw colour outside `tokens.css`, and never name an elevation shade directly. `scripts/check_css_tokens.py` fails the build on all of them (wired into `make lint-js` and pre-commit).
...
- **Spacing is exactly five steps, and every `gap` / `margin` / `padding` references one:** `--sp-1` (4px, intra-control — a label to its box, a glyph to its text) · `--sp-2` (8px, control padding and inline groups) · `--sp-3` (12px, inter-control and card padding) · `--sp-4` (24px, inter-field and inter-column rhythm) · `--sp-5` (32px, major cluster separation). Replaced ~150 free-chosen values spread 0.75–22.5px across 17 stylesheets, snapped to the nearest step with ties rounded up. The gate rejects any **bare length** in a spacing value rather than demanding every component be a token — so `0`, `auto`, and calc() arithmetic (`calc(var(--sp-3) * 1.1)`, `calc(var(--sp-4) / -2)`) stay legal, while a half-tokenised `gap: var(--sp-2) 4px` fails. A value that does a *job* rather than set rhythm — clearance under the fixed pending bar, room for a `appearance:none` caret, a size that must track its own text — takes `/* token-exempt: <reason> */`; there are six.
