...
**Typography and surfaces are tokens-only (binding).** Never write a literal `font-size`, `font-weight` or `letter-spacing` in `static/css/`, never a raw colour outside `tokens.css`, and never name an elevation shade directly. `scripts/check_css_tokens.py` fails the build on all three (wired into `make lint-js` and pre-commit).
...
- **Two-column cards carry a centred rule (binding).** *Every* full-width card with two internal columns draws a 1px `var(--line)` hairline between them, and **every such rule sits on the card's centre line** so they stack vertically down the page. Three mechanisms, by shape:
  ...
  - A full-span row (`.span`) must not be struck through — it masks the rule over its own height by painting its surface (`--surface-card` inside any card-like container, `--surface-page` on the bare page). A new card-like container must be added to that mask's selector list, or the row paints page colour over a card and reads as a dark band.
