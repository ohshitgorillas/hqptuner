...
Open: Mercury, and whether Dark Side's tracks count in the 20k grade. No engine port before the rule is locked.
...
At -15 dB the veto costs no fake block; at -18 dB it starts to take Wine Dark Sea, median -21.4. The locked veto is -15 dB, yielding to a loud-frame step of 14 dB or more (section 3.2). Its real-side gain is 4 blocks, and its cost is The Patient, a fake whose junk above the fold sits within 7 dB of its music: 15 of that track's 20 blocks are forced real at a median ratio of -6.9 dB. The sweep above reads The Patient as real, so its real-error column counts those blocks on the wrong side. Its reason is the one-sided cost: undecided blocks must fall to `OFF`, so fake is proven before engaging and real evidence overrides. With the -125 dB line and the -15 dB veto G's residue is 47 blocks of 1085: Airbag 20, Caress of Steel 13 and Foundations of Burden 10 left alone, 4 real blocks called junk. The loud-frame step below takes that to 23.

**What the residue looks like to the step.** Median signed step at the fold over kept blocks, above minus below, so a wall reads negative. Fakes caught: Vile Nilotic Rites -40.8 dB, The Hot Rock -24.4, The Past Is Still Alive -15.1, Wine Dark Sea -13.3. Fakes missed: Foundations of Burden -8.5, Airbag -6.0. Caress of Steel, fake, -6.4, and The Patient, fake, -8.8. Reals: Anhedonia -5.9, Bitches Brew -5.4, OK Computer's real tracks -1.9 to -3.3, most others -1 to -4. Airbag's minimum-curve step sits inside the real range; Foundations, The Patient and the loudest reals read the same size, so no cut on the step separates them and the ratio veto is what tells them apart.

**Loud-frame step**, candidate G90, locked at 11 dB. G90 is G's fold step on the per-bin 90th percentile curve instead of the minimum. Over kept blocks: fake median 11.9 dB, p10 3.1; real median 4.1, p90 8.4, p100 21.4. Per-album medians: Vile Nilotic Rites 35.4, The Past Is Still Alive 31.9, The Hot Rock 26.4, Wine Dark Sea 17.1, Airbag 15.2, Foundations of Burden 12.0, The Patient 12.8, Caress of Steel 10.5; reals Let Down 9.5, OK Computer's other real tracks 2.5 to 5.8, the rest under 7. Caress of Steel is fake: G reads it -6.4 on the minimum curve and misses all 13 kept blocks, the loud-frame step reads it 10.5. Leave-one-album-out with the veto, Celestial Blues as real: G 47 wrong, 43 fake called real, 4 real called fake; G90 34, 22 and 12; either fires 34, 21 and 13. The held-out cut for G90 sits near 14 dB, since each fake album in play sits at a different height. Fixed cuts over the whole corpus:
...
| 12 dB | 41 | 38 | 3 |
| 14 dB | 48 | 47 | 1 |
| held out, one album at a time | 36 | 28 | 8 |

At 11 dB the fake side is The Patient 16, Caress of Steel 8, Airbag 5, Foundations of Burden 2. The real side is one block each on Is It Now?, Meddle, Thrice Woven and Subterranean Homesick Alien, none of which reaches the filter under the policy's confirming block (section 3.2). The Patient's other 4 blocks, where its ratio dips under the veto, are catches. The held-out cut reads one block worse than the fixed 11 dB and one burst better under the policy below; the two cuts are the same rule to within the corpus.

**Policy grade**, per unique steady burst, a burst engaging on two consecutive junk blocks, a dropped or vetoed block breaking the run. Of 135 fake bursts, 94 carry fewer than two blocks above the line and are settled `OFF` by the mask alone; 33 are in play, against 231 real bursts. 12 of the 33 belong to Lateralus, Awaken My Love, CALIGULA and Foundations of Burden, all fake.

| Rule | Veto | Real engaged | Fake engaged | Fake missed |
|---|---|---:|---:|---:|
| G at its cut | -15 dB | 0 | 17 | 16 |
| G90 at 11 dB | -15 dB | 0 | 22 | 11 |
| G90 at its held-out cut | -15 dB | 0 | 23 | 10 |
| Either fires | -15 dB | 0 | 23 | 10 |
| G at its cut | none | 0 | 20 | 13 |
| G90 at 11 dB | none | 1 | 25 | 8 |
| G90 at its held-out cut | none | 1 | 26 | 7 |
| G90 at 11 dB | -8 to -12 dB | 1 | 22 | 11 |
| G90 at 11 dB | -18 dB | 0 | 17 | 16 |
| G90 at 11 dB | -21 dB | 0 | 13 | 20 |
| G90 at 11 dB | -15 dB, yielding at 12, 13 or 14 dB | 0 | 25 | 8 |
| G90 at 11 dB | -15 dB, yielding at 16 or 20 dB | 0 | 22 | 11 |

**Yielding veto.** A block whose loud-frame step reaches the yield value engages whatever its ratio. At 12 to 14 dB it takes every Lateralus burst that shows a wall, The Patient at 267 s (steps 11.5 to 18.8 dB, ratios -4 to -22) and 393 s (15.4 to 16.5, -7 to -8) and Schism at 253 s (11.2 to 14.4, -1 to -2), and leaves the two that do not, The Patient at 16 s and Schism at 0 s, steps under 2 dB with the ratio positive: floor through the fold, nothing to remove. Meddle's one burst reaches 13.2 dB on a single block, which the confirming block absorbs. The locked yield is 14 dB, the top of the flat range, one dB of real-side margin over Meddle with Schism's wall blocks at 14.3 and 14.4 still consecutive. With it the 8 missed bursts are 2 correct `OFF` by the ruling in section 2 and 6 others, and block by block those 6 are not step misses. Foundations of Burden at 133409Z and 231047Z and Wine Dark Sea at 81102Z carry steps of 12 to 22 dB on every block and are missed because the mask drops every other block, above-fold level sitting on the -125 dB line, and a dropped block breaks the run: kept blocks 1 and 4, 2 and 4, 0 and 2, never two in a row. Caress of Steel at 50041Z reads 10.5, 7.3 and 15.6 dB on its kept blocks, the first half a dB under the cut. Foundations at 133404Z reads 10.99, 11.85 and 10.66, two blocks under the cut by 0.01 and 0.34 dB. Caress of Steel at 45630Z is the one veto miss left, steps 6 to 10 dB with ratios near -10 dB. The statistic sees three of the six on every block.

**Slope break at the fold**, candidate T as `flip`, open. On the p90 curve after the 9-bin smooth, the least-squares slope in dB per kHz over the 1.5 kHz band ending 300 Hz below the fold, `below`, and over the 1.5 kHz band starting 300 Hz above it, `above`; `flip` is `above` minus `below`. A fake's source falls steeply into its own Nyquist, the anti-alias roll-off of the 1x master, and its images above fall gently, so `below` is steep and `flip` positive; a real converter's curve has the same gentle slope on both sides. Per-album medians over kept blocks: Vile Nilotic Rites 10.7, Wine Dark Sea 6.4, Airbag 5.0, The Grudge 4.6, Foundations of Burden 3.1, Caress of Steel 2.6; reals -0.7 to 1.1 except Anhedonia 3.2 on 7 blocks and Dig Me Out 2.2 on 5. It misses the fakes whose stopband falls inside the window, The Patient -2.0, The Hot Rock -1.8, Awaken My Love -7.2, The Past Is Still Alive -24.2, and reads Schism 0.6; those are the loud-frame step's catches, so the two are complementary where the step is weak. Block by block on the residue: Caress of Steel at 45630Z reads 2.4 to 4.5 on every block with steps of 6 to 10 dB; Meddle's burst reads 1.5 to 3.3 on steps of 9 to 13, the closest real. The sign test built from the same slopes never fires (section 6). Graded as a qualifier under the policy, step at or over 11 dB or at or over a floor with `flip` at or over a cut, veto yielding at 14 dB: floor 8 dB and cut 4 dB per kHz engages 26 fake bursts to the working rule's 25 and no real burst, the one gain Caress of Steel at 50041Z; cut 3 adds Foundations of Burden at 133404Z and engages three bursts of Is It Now?; floor 6 at cut 4 adds nothing and engages Thrice Woven once. Alone it is 102 wrong blocks at 4 dB per kHz against the step's 35. One burst at the top of the swept range, with the real-side margin beyond 4 untested, is what `flip` is worth; it stays open, not adopted.

**Per-frame fold step**, candidate FS, open, the first 20k statistic read on single frames. Per raw frame, at each fold in 22.05 and 24 kHz, the median over the 1.5 kHz band ending 300 Hz below the fold minus the median over the 1.5 kHz band starting 300 Hz above it; a frame is lit when its per-Hz mean over 300 Hz to 6 kHz above that fold reaches -125 dB, and an unlit frame carries no step. A block reads junk when it carries at least a floor of lit frames and the share of lit frames whose step reaches a cut is at or above a share cut; veto and yield kept as above. 355 steady bursts, 1765 blocks. Foundations of Burden lights 1857 of 5663 frames, median lit share 0.284 per block, lit-frame step p10, p50, p90 of 4.9, 10.2 and 19.8 dB; reals light near fully at p50 steps of 0.8 (Meddle) to 4.6 (Bitches Brew), Is It Now? p90 7.3, Dark Side p90 9.1. Five fake albums light no frame at all, AVOW, Amassakoul, Amnesty, Guidance and Marked for Death, so FS never grades them; they are the deep-stopband fakes the mask already leaves `OFF`.

| Floor | Step cut | Share cut | Real engaged | Fake engaged | Fake missed |
|---:|---:|---:|---:|---:|---:|
| 5 | 6 dB | 0.3 | 25 | 43 | 83 |
| 5 | 8 dB | 0.5 | 2 | 33 | 93 |
| 5 | 10 dB | 0.3 | 2 | 35 | 91 |
| 5 | 10 dB | 0.5 | 1 | 23 | 103 |
| 5 | 12 dB | 0.7 | 0 | 15 | 111 |
| 10 | 8 dB | 0.5 | 2 | 26 | 100 |
| 20 | 10 dB | 0.7 | 0 | 8 | 118 |

Against the shipped rule's 28 fake and 0 real, FS at floor 5 reaches 33 to 35 fake for 2 real bursts, and 15 fake at 0 real. The 2 real bursts at 8 dB and share 0.5 are One Beat, every block junk with shares 0.70 to 0.80, ratios -18 to -23 and G90 12.5 to 15.5, and The Dark Side of the Moon 50th, engaging on blocks 1 and 2 with shares 0.56 and 0.76 while the veto holds the other three. Both are fake, so both engages are catches and FS's real cost at those cuts is zero bursts against the shipped rule's zero, with 35 fake engaged to its 28. Dark Side is 115 blocks that G reads clean, a 192 kHz container whose steady content rolls off about 20 dB from 16 to 26 kHz; the shipped rule never sees it because the mask drops its blocks, and FS sees it in a third of its bursts' blocks. Veto off, the sweep moves by at most five real bursts at 6 dB and by one burst elsewhere. Foundations at floor 5 engages 10 of 12 bursts at 6 dB and share 0.3, 9 at 8 dB and 0.5, 8 at 10 dB and 0.3. Lit gate is the mask's -125 dB line applied per frame, so the fakes the mask never lit stay unlit here.

Under the policy no rule with the -15 dB veto engages a real burst anywhere in the corpus. The veto's whole cost is five bursts of Lateralus, The Patient at 267 s and 393 s and two more, Schism at 0 s and 253 s, whose junk sits within 8 dB of the music: the loud-frame step reads Schism 14.0 dB and The Patient 12.8, well over the cut, and 13 of Schism's 20 kept blocks and 16 of The Patient's are forced real at every cut from 8 to 11 dB, so the misses are the veto's alone. Its whole gain is one burst of Meddle that the loud-frame step engages without it. The Grudge, 12.0 dB, engages on all three of its in-play bursts. The missed fakes at the working rule are Caress of Steel 2, Foundations of Burden 3, Schism 2, The Patient 3, Wine Dark Sea 1. Both-fire reads as G alone and either-fire as G90 alone, so the two statistics nest: every burst G engages, G90 engages too.

The locked 20k rule is therefore: block kept at or above -125 dB per Hz above the fold, loud-frame step over its cut, and either the ratio to the music under -15 dB or the step at or over the 14 dB yield; filter engages on the second such kept block within an index distance of 3, holds through dropped blocks, and drops on the first kept block that reads real (section 3.2). Celestial Blues, 10 kept blocks, reads real on every statistic.
...
A fake burst whose content never reaches the source Nyquist, so that one noise floor runs through the fold with nothing above it to remove, is correctly `OFF`: the filter would trade floor for floor, and the master is fake in name only until the music climbs into the wall. The Patient at 16 s is the reference case; at 267 s and 393 s the same track shows its wall and is a miss. A burst-level miss count that charges these bursts to the detector overstates the residue, and the policy grade does not yet separate them.

Bursts that span a track boundary are kept and scored in their own table, since they are the release test.

Corpus, under `/srv/hqptuner/state/junkburst/`: 528 bursts, labelled by artist and album in `labels.tsv` and joined to burst stamps through Roon history in `tracks.tsv`; three albums are labelled by track: Dragon New Warm Mountain I Believe in You with Simulation Swarm fake and Love Love Love and Change real, OK Computer OKNOTOK with Airbag fake and Paranoid Android, Subterranean Homesick Alien, Exit Music (For a Film) and Let Down real, and Lateralus (Hi-Res Remaster) with The Patient, The Grudge and Schism fake, every track of it heard so far. Caress of Steel is fake. One Beat is fake, a wall at 21.5 kHz in every frame with a flat converter floor and four spurs above it. The Dark Side of the Moon 50th Anniversary is fake, 115 blocks. Bitches Brew is fake. Every burst has a `tracks.tsv` row; 4 reach no label. Two captures fail their gzip CRC and carry no derived pair. Roon's logs rotate, so 275 of the 492 track rows predate what the logs can rebuild, and the join runs right after each round. A real-side error concentrated on one album is a label to re-examine before it is a detector defect. Section 3's 1612-block tables pool OK Computer's 14 bursts, 6 of them Airbag, under one fake label; every section 2 figure grades it by track. 30 track matches are low confidence, and a burst with no Roon line has no track identity and no label. A capture round grows the corpus; each new round is graded once at the standing cut before it joins the cut pick.

Bursts whose 5 s spans overlap are the same audio captured by more than one instance of the capture script and collapse to the earliest stamp before scoring. The whole corpus collapses to 366 unique labelled bursts, 135 FAKE and 231 REAL. The block counts and every table above the policy grade are read over the 334 bursts that carried labels when they were last rerun, which happens only when a cut moves. A label other than `FAKE`, `REAL` or `CLEAN` drops the burst from every table; `CLEAN` grades as real.
...
**Policy.** Changing the junk filter on the running engine costs nothing: no interruption, no glitch, no silence. So the 20k setting follows the blocks. It engages where a block reads junk above the fold and drops where the next block reads nothing to remove or music, a clean passage on a fake included. Bleed into real content is capped at 4 s, which at 1 s blocks and a 1 to 2 s poll leaves room for one confirming block and no more. A sample-rate change resets state at once. The 30k and 40k settings keep a hysteresis of their own on a 3 to 5 s minimum (section 4).
