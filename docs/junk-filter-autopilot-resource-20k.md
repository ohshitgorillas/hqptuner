...
**Eyes against the rules, same 48 bursts.** The shipped rule as `hqptuner/engine/junkrun.py` runs it and candidate FS at 8 dB share 0.5 and 10 dB share 0.3, lit floor 5, graded per burst by `.junkburst-eyes/detector.py` over the headline blocks, joined to the readings in `.junkburst-eyes/agreement.md`. No rule engages on a burst the eyes call absent, and none engages on a real burst. On the 12 fakes with junk above the line, all eyes-present, the shipped rule engages 5 and FS engages 7, adding Foundations 133614Z and Wine Dark Sea; the two FS settings give the same verdict on every burst. The shipped rule's misses are mostly `thin`, not `no`: the -125 dB mask leaves fewer than two gradable blocks on 17 of its 31 misses, so it never grades them, and the mask line, not the step, is what fails on that side. FS's misses that matter are bursts the eyes call dense or faint with the wall near 22 kHz and no positive block: Foundations 133815Z, both Paranoid Android bursts, and one block only on Worlds Apart 231253Z and CALIGULA 204004Z. Under the per-track latch FS covers Foundations of Burden, Worlds Apart, Wine Dark Sea, Vile Nilotic Rites and The Hot Rock, and still misses OK Computer and CALIGULA.

**Status.** FS's per-frame positives follow the eyes and cost nothing on real, so it is the candidate to replace the loud-frame step, with the per-track latch as the engage and the grade unit. Its open defect is inside the rule: three bursts the eyes call dense score no positive block, cause unmeasured. Next is a side-by-side of FS's per-frame internals, lit count, step and share, on those stamps against an engaged Foundations burst, with no cut changed until that is read. No engine port before then.
