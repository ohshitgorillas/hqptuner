...
Gap is the index distance between the two confirming kept blocks, so adjacent blocks read gap 1. No setting engages a real burst. Gap 3 spans a 5 s burst, so gap 5 adds nothing. The hold bound never bites: no burst carries a drop run longer than 3, so hold is ungraded by this corpus; hold is unbounded, since a dropped block on a real album engages nothing and the filter drops on the first kept real block regardless. The locked gap is 3, hold unbounded. It recovers the three run-rule misses, Foundations of Burden at 133409Z (kept blocks 1 and 4) and 231047Z (2 and 4) and Wine Dark Sea at 81102Z (0 and 2). Caress of Steel at 50041Z is not a run-rule miss: its kept blocks read 10.5, 7.3 and 15.6 dB, and the first, under the 11 dB cut, reads real and breaks the run. The five misses left are two correct `OFF`, The Patient at 16 s and Schism at 0 s, and three blocks under the cut by half a dB or less, Foundations at 133404Z and Caress of Steel at 50041Z and 45630Z. A 10 dB cut would take them at the price of 12 real blocks over the line, which under gap 3 is enough to engage a real burst, so the cut holds at 11 dB.
