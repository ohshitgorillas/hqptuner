...
2. **One assertion per test.** Each test asserts one condition (`assert` statement or one `pytest.raises` block). Failure names one broken behavior. Case sweeps use `@pytest.mark.parametrize` — one condition per generated case — never assert-in-a-loop or assertion stacks. A boolean conjunction is one assertion per operand, and `all()` or `any()` over a generator is an unparametrized sweep. `is not None and x["k"] == v` is the common dodge; strict mypy runs over `tests/`, so the repair is the empty value of the declared type in place of `None`: `(x or {})["k"] == v` for a mapping, `(x or [])[i] == v` for a sequence, `(x || {}).k === v` in JS. A `None` then fails on the missing key with one assertion and no ignore. `scripts/gates/testing/check_test_assertions.py` and `eslint-rules/assertion-shape.js` report the conjunction, root-only.
...
7. **No test waits on wall clock.** Retry/verify/poll loop tested for how many passes it makes and what it concludes — never how long it takes. So production code paces itself through injectable clock, suite virtualizes it.
   ...
   - **The discovery sweep paces on `discover`'s own seams**, its `clock` and `sleep` keyword arguments, defaulting to `time.monotonic` and `asyncio.sleep`. A test injects an advancing pair through the entry point; the interval between reads is `hqptuner.engine.discovery.POLL`, so a window is stated in poll intervals rather than in seconds.
   ...
   - **Gated, three ways**: `scripts/gates/testing/check_suite_time.py` holds the offline suite's wall time to the last green run's — over 5 s slower fails and escalates to the owner (`--accept` records it), 10 s or more slower is rejected outright. `scripts/idle_probe.py` records each test's wall time less its CPU time and `scripts/gates/check_idle.py` refuses any test whose smallest recorded idle exceeds 40 ms. `scripts/gates/testing/check_test_clocks.py` reads the source instead of the clock: a `time.sleep` or `asyncio.sleep` on anything but the literal `0`, and a `timeout` or `*_timeout` given a literal between zero and 0.5 s, fail it outside `tests/e2e/`.

8. **New tests must bite.** A test written for new or changed behavior must fail against the pre-change code — a test that is green both with and without the change constrains nothing, however well-shaped it looks to the mechanical gates. The chain enforces this with a bite check: implementation reverted to HEAD (tests kept), new tests re-run, red expected. Assertion failure is the only result that proves bite. A collection or import error proves the test names a symbol that does not exist yet, which every test of a new surface does regardless of what it asserts; it is not weak evidence, it is no evidence. Where the surface is new and no red run can say anything, the obligation is discharged statically instead: the spec block names the null stub each line fails — the module present, exports named, every function returning its zero value — and mutation testing is what checks it later. Tests with no pre-change state to fail against — characterization of existing behavior, tests accompanying a pure refactor — are exempt, and the exemption is stated in the hand-back rather than assumed silently; mutation testing (below) covers those over time.

9. **A test asserts only strings it put on the wire itself, wire identifiers, and numbers derived from wire data. Every string born inside `hqptuner/` is copy.** Source or `data/`, makes no difference: labels, headings, blurbs, hints, tooltips, status lines, error text, a picker's option list, a curated list's order and its count are owner-owned data, reworded at will. The test: to know this literal, would the writer have to read `hqptuner/`? Then it is copy, and it stays out of the assertion and out of the DOM selector. A device name the fixture handed in is wire data and may be asserted; the sentence the app wraps around it may not. Wire identifiers — engine names, option values, config attributes, JSON keys, state-bearing CSS classes (rule 11), error codes — are contract, and pinning those is correct. Consequences:
   ...
   - Where a selector would need a sentence, add a `data-testid`. If nothing meaningful survives removing the wording, delete the test instead of leaving a tautology — through the chain with `motion: strike`, which is the route that performs a deletion, since no hand edits `tests/` directly. `scripts/gates/testing/check_no_copy_assertions.py` and its eslint peer catch the mechanical shape; the principle is what review checks.
...
10. **A test discriminates, or it is a tautology.** The question a test answers is not "does the code do something" but "which of these two implementations am I looking at". An assertion the wrong implementation also satisfies constrains nothing, and it costs the same to run and maintain as one that does. The shapes that fail this, all of them common and all of them green:
    ...
    - **Asserting existence instead of value.** Truthiness, `is not None`, a type check, a length, a key's presence. Legal only where existence *is* the contract and the value is genuinely unbounded. Such a site is an entry in the gate's `EXEMPT` table with its reason, owner-approved like every exemption; every other existence-only assertion is reported.
    - **Asserting a value the absent feature also produces.** `0`, `None`, `""`, `[]`, `False`, the default the fixture already carried. If the expected value equals the zero value, the test passes against a feature that was never written.
    ...
    - **Restating the implementation's arithmetic.** A test computing the expected number the same way the code does asserts that Python is deterministic. Write the number.

    The check is mechanical: name an implementation that fails the assertion and one that passes it, both plausible. If the failing one is only "the feature is absent", the test pins nothing beyond presence. Where the unit is pure mathematics this check has no repair — every numeric assertion restates an identity the implementation was derived from — so such a module is pinned against an external reference oracle committed as fixtures, not against a spec block.

11. **A test guards against a bug, never against a change of mind.** Before a test is kept, write the bug report its failure would file. "Preset list came back unsorted" is a bug; "preset list is no longer alpha, mike, zulu" is a diff. If the only way to turn the test red is for the owner or a forker to decide differently, it pins a decision: reshape it into the invariant the decision serves, or delete it. The shapes:
    ...
    - **Styling as contract.** A class or attribute that carries state (`active`, `off`, `disabled`, `data-*`, a `data-testid`) is contract. A class or wrapper that only selects a stylesheet rule is design, and so is nesting order unless the nesting is the behavior. Rule 9's "CSS classes" means the first kind.
    - **A default as an absolute.** The owner's default for a knob is a decision. Test its role: an unset facet reads as the default, a written one overrides it. HQPlayer's own defaults are wire facts and may be pinned.
    - **A sibling status code.** Which 4xx a refusal returns is contract only where a client branches on it or the OpenAPI snapshot carries it; otherwise pin the `code` and the status class.
    - **Formatting.** Rounding, key order, indentation, the string form of a number, unless a wire demands it.

    The tell: a value the test's own fixture put on the wire may be asserted back verbatim; a value the design chose is not the fixture's and is not asserted absolutely. Rule 10 asks for two distinct expected values on a surface; satisfy it with two inputs to the invariant, never two design literals.
...
14. **Helpers return values.** No `assert` lives outside a `test_*` function. A helper returns the evidence and the test asserts it; a fixture that must refuse to run raises. The gate reports an assert in any module-level function that is not a test.

15. **Lowest lane.** A behavior is tested at the lowest lane that observes it: pure function, then store, then REST, then rendered component, then browser. An e2e test a unit test already covers is deleted.
...
- A `skip` or `xfail` decorator on a test, or any skip whose reason is that the test is broken, flaky or unfinished, is a gate exemption: owner-approved, in `EXEMPT`, reason naming the condition that removes it. A skip on an environment or data precondition the reason names (`importorskip`, an unreachable daemon, an empty shipped mapping) is this section's own mechanism, not an exemption.
...
- **One assertion per test enforced, not merely asked for.** `eslint-rules/one-assertion-per-test.js` is peer of `scripts/gates/testing/check_test_assertions.py`. Does not look inside nested functions, so assertion wrapped in helper or `.then()` callback counts as **zero** and is flagged. Deliberate: gate you defeat by moving assert into function is not a gate. Keep assert at call site — if helper builds condition, have it return `[ok, message]` and spread into one `assert.ok(...)`. The `assertion-shape` rule reads the shape of the one assertion the count rule counts, root-only.
...
- **Components render through `preact-render-to-string`.** Assertions on rendered output — state-bearing classes, attributes, `data-*` state, and text only where it is a wire identifier or a number (rule 9) — never internal flags.
