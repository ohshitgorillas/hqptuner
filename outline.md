...
4. Verify the signal path bar's processing block order (§3) against the stock web UI's block display — the original mockup order was wrong per Jussi; current §3 order is manual-derived, not yet observed.
