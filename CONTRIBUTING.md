...
There is deliberately no per-package line total. One was tried and removed: pinned at each package's current size it froze the tree, so the only ways past it were raising the number, moving code into an unbudgeted package, or cutting documentation to buy room — every escape worse than the sharding it was meant to catch. Sharding is caught by shape instead, below.
