...
- **Preset actions report failures under the signal path.** The header's preset row no longer prints status text or a pending marker, and a pick or delete that fails now shows a warning on the alert strip, which is on screen in LIVE as well.
