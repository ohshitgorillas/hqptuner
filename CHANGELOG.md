...
- **LIVE mode.** The switch in the header replaces the tabbed view with one page holding the settings the running engine can change in place: output mode and rate, the loaded chain's two filters and its dither or modulator, the high-frequency playback filter, adaptive volume, playback volume, and the matrix profile. There is no Apply and no staging — each control writes when you change it, and the value shown afterwards is what the engine reported back rather than what was requested. A change can interrupt playback. Nothing on the page is saved: these settings last until the daemon next restarts, including the restart an Apply in the tabs view performs. Both filter chains are on the page as the same pair of collapsible **PCM Chain** and **SDM Chain** cards the Resampling tab carries — the one the output mode uses open, the other collapsed, both open in auto. The card for the chain the engine has not loaded still takes edits: HQPlayer can only be told about the chain it is running, so those are held and applied the moment that chain loads, which means you can set up the SDM chain while PCM is playing. Controls whose write re-enumerates the engine's lists (mode, either filter, the rate) reload those lists before the next control reads them. Staged edits waiting in the tabs view show as a chip; LIVE does not flush them. Each control's label, note and per-selection description is the text its twin carries on its tab. The engine-health card is on LIVE too, under mode and rate; the page polls at 500 ms and has no "quick updates" tickbox, which stays on the System and Volume tabs.
