...
- **Rounded corners now come in four sizes instead of seventeen.** Every corner in the interface was picked by hand, which is how a chip ended up rounder than the chip beside it for no reason anyone could name. There are now four sizes chosen by what the thing *is* — a hairline mark, a small chip, a control, or a panel that holds other things — plus fully-round and circular. Nothing moves by more than about a pixel and a half; the point is that the next control added cannot land a shape of its own. The build now refuses a hand-picked corner, and likewise a hand-picked animation speed: those are down to two, one for things reacting to you and one for the slow settle of a live meter.
