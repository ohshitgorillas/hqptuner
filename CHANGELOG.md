...
- **An apply no longer stores the departing daemon's settings into a preset.** A restore restarts the daemon seconds after it returns, and readings taken in that window described the process being replaced — which could write another preset's mode and modulator into the active one and then apply them as the running config. Applies and preset loads now wait for the restarted daemon before reading anything back, and report the apply unconfirmed when it never arrives.
