...
- **A band strip under the matrix response plot.** A standing block of sliders with exact-entry boxes — frequency (log scale), gain, and Q / bandwidth / slope — always visible under the plot, disabled until a band is bound. Select a parametric stage (click a stage chip, or grab a dot on the plot) and the same rows go live in place, so nothing shifts under your cursor mid-drag. This is the first way to adjust Q visually; dragging the dot only ever moved frequency and gain. Sliders stream into the plotted curve live and commit on release; the boxes take exact figures. Stereo-paired pipelines move together, same as dot drags, and the strip is named for what it edits ("1+2 · peak"). Grabbing a dot on the plot selects its band, so the strip docks straight from the plot — no trip back to the pipeline chips. Biquad and non-parametric stages keep the docked text editor only.

### Fixed

- **Dragging a stereo-paired EQ dot no longer leaves a ghost point behind.** A stereo pair plots as one merged "1+2" curve with its dots exactly overlapping — but the live drag preview moved only the grabbed pipeline's copy, so the moment you dragged, the pair split: the curve doubled and the twin's dot stayed at the grab point, looking like a brand-new point had been created. It re-merged on release, ghosting again on the next drag. The drag preview now moves the byte-identical twin live, the same rule the release has always committed by.

- **Grabbing an EQ dot no longer nudges it.** The drag handles on the response and loudness plots treated the pointer's position as the value, so a press slightly off a dot's centre snapped the dot under the cursor — and even a plain click committed that nudge. A drag is now relative to where you grabbed: the dot holds its value until the pointer actually moves, then follows by exactly how far it moved.
