...
- **A LIVE mode, for changing what you're hearing right now.** The switch beside the tabs replaces the whole tabbed view with a single page of only the settings the running engine can change on the spot: output mode and rate, the loaded chain's two filters and its dither or modulator, the high-frequency playback filter, adaptive volume, playback volume, and the matrix profile. There is no Apply and no staging — each control writes the moment you change it, and the value you then see is what the engine reported back, not what you asked for. Nothing on the page is ever refused because playback is running; a change that interrupts what you are listening to is yours to spend, and the page says so up front rather than greying the control. Nothing on it is saved, either: every one of these lasts until the daemon next restarts, including the restart an Apply in the tabs view performs. Which filter controls exist follows the chain the engine actually loaded — PCM or SDM — and in auto mode before playback starts the card says neither is known yet instead of guessing one. Controls whose write re-enumerates the engine's lists (mode, either filter, the rate) reload those lists before the next control reads them. Staged edits waiting in the tabs view show as a chip so an owed Apply is not forgotten; LIVE never flushes them. Every control's label, note and per-selection description is the same text its twin carries on its tab.

- **The PCM and SDM rate menus now offer Auto**, on the Output tab and on LIVE. It hands the rate to the engine, which picks the highest one the filter and DAC can reach per track. The fixed entries pin the rate instead, so a 44.1 kHz track can no longer climb past it.
