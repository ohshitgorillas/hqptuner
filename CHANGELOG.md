...
- **Coming back to Headphones restores the crossfeed the Speakers switch suppressed.** Switching to Speakers correctly took the crossfeed out of the signal path; switching back left it out, so a trip through the speaker controls silently cost you a setup you had built. Suppression is not a decision to abandon the setting — crossfeed belongs to the headphone side, and the headphone side is where you left it. The switch now snapshots what it takes (the pipelines as they stood, the crossfeed flag as it stood) and puts exactly that back on return: a structural block returns as a block, Bauer's flag and its compensation rows return together, and a crossfeed that was already off stays off, because the switch never took it away. Nothing is restored over pipelines edited on the speaker side — that work is yours, and the snapshot is dropped instead. The snapshot rides in browser storage next to the mode itself, so reloading the page on the speaker side does not strand the headphone setup.

- **Switching the DSP tab to Speakers now takes the whole crossfeed with it, and leaves the EQ profile alone.** It switched off HQPlayer's crossfeed flag and removed a structural block, but ignored Bauer's compensation block — eight matrix pipelines whose entire purpose is to correct for a crossfeed that had, one line earlier, just been switched off. They stayed in the signal path on speakers, applying a mid/side correction to a crossfeed that was no longer running. Worse, those eight rows are where the loaded headphone EQ lives, so the profile went with them: what came out on the other side of the switch was not the EQ the user had loaded. Switching to Speakers now dismantles that block back to the plain stereo pair, which puts the EQ profile back on pipelines 1+2 exactly as it was — and, as before, switching back to Headphones re-enables nothing.
