...
- **The interface now speaks a finite visual vocabulary, and the build enforces it.** Five things had each drifted into dozens of one-off choices made by eye; all five are now short lists picked by role. **Corners** went from seventeen hand-picked radii to four sizes plus fully-round and circular, chosen by what a thing *is* — a hairline mark, a small chip, a control, or a panel that holds other things — so a chip can no longer end up rounder than the chip beside it. **Motion** went to two speeds: one for things reacting to you, one for the slow settle of a live meter. **Spacing** — every gap, margin and inset — went from roughly thirty values scattered between three-quarters of a pixel and twenty-two down to the same five steps (4, 8, 12, 24, 32 pixels) the page layout already used. **Fading** — every place the interface dims something — went from thirteen strengths to six, split by what the dimming is for: three that say whether a control is in play (cannot be used, switched off, or a whole section that is off but still worth reading) and three that say how far back a line sits in a plot or diagram. Nine of the thirteen were all trying to say the same thing, "switched off", which is why one idle chip could fade harder than the idle bar beside it. And **a collapsible section is now literally a card**: the same component with a switch that turns its header into a toggle, rather than a second piece of code styled to resemble the first — which is exactly how the two drifted into different shades to begin with. What you will actually see: most control padding lands on the smallest step, so buttons, chips, dropdowns and input rows sit a little tighter; anything that had been sitting between 13 and 22 pixels moved to 12 or 24; corners shift by at most a pixel and a half; the signal path across the top reads a little stronger while playback is stopped, phase traces and the crossfeed angle arc step forward slightly, and plot grid lines recede; and sections that collapse (the backend sections on Output, the filter sections on Resampling, About HQPTuner on System) carry the card header's dividing line and the card's roomier inner padding, with the same gap between stacked cards that a collapsible always had. A handful of values that do a job rather than set rhythm are deliberately left alone and now say why in the code — the clearance that keeps the last row from hiding behind the pending bar, the room a dropdown's caret needs, the few sizes that must track their own text rather than the page. The point of all of it is the next control: it cannot arrive with a shape, a speed, a gap or a frame of its own, because the build refuses anything that is not from the named set.

- **Frequencies and levels now read the same way everywhere.** The same figure used to be written several ways depending on which card you were looking at: a rate appeared as `1 kHz`, `1.0 kHz`, `1.00 kHz` or `1000 Hz`, and a decibel figure carried a leading `+` in some places and not others with no rule behind which. One place in the code hand-wrote the plus sign that another place already had. Rates now pick their unit the same way in every readout and drop trailing zeros, so a shaper that needs a higher rate says `192 kHz` rather than `192.0 kHz` and an uploaded filter says `48 kHz` rather than `48.0 kHz`. Decibels now carry a sign only where the number is an offset from somewhere — the EQ readout while you drag a point, the crossfeed's center shift — and never where it is simply a level, so a plain level can no longer turn up signed on one card and unsigned on the next. How many decimals a figure shows is still per-readout, deliberately: a value you watch move under the pointer holds its decimals so the line doesn't jump around as digits come and go, while a figure you read once shows the shortest true number.
