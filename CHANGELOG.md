...
- The Resampling tab's PCM and SDM cards stopped auto-expanding once you had collapsed one by hand. A manual collapse set an override that outranked the mode-driven disclosure permanently, so a card you closed in Auto mode stayed shut even after you switched to the mode that uses it — select PCM, and the PCM card you had collapsed earlier would not reopen. The manual override now clears on every mode change, so each switch re-asserts the automatic open/closed state; a hand toggle still wins until the next mode change.
