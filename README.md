...
**3. Adjustable crossfeed EQ compensation.** Crossfeed tilts tonal balance — it dulls centered sound: vocals, bass, most of the mix. The polished implementations (Goodhertz CanOpener, Meier) solve this by building compensation into their own DSP; HQPlayer's Bauer crossfeed is a simple, uncompensated three-preset model with no such option. HQPTuner compensates it from the outside — something we haven't seen done elsewhere: it reads your configured crossfeed parameters, generates matched inverse EQ stages (HQPlayer's crossfeed is libbs2b — attributed in its own manual's license list — and the math is verified against the bs2b source; ~1–2.7 dB of center tilt depending on settings), and compiles your headphone EQ into a mid/side pipeline block that corrects exactly the center coloration while leaving the crossfeed's stereo width effect untouched. A strength slider (less for hard-panned recordings, full for center-heavy mixes), a live correction plot, and a "what you hear" overlay show the whole thing on screen — and the block stays linked to your crossfeed settings: change them and it flags itself stale for a one-click rebuild.
