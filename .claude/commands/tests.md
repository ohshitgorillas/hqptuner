...
**`kind: excision` is the one block that removes tests instead of adding them**, and it carries excision lines in place of behavior lines: no behavior lines, no replacement tests, no mixed block. It exists because `docs/testing.md` rule 9 orders a copy-pinning test deleted and nothing else in this repo can carry that order out. Each line names one target, cites the rule number it violates, and quotes the offending assertion:
...
The target is `tests/<file>::<test>` for a pytest test, `tests/js/<file>::"<test title>"` for a node one — a spec-block identifier the writer resolves, never a runner invocation — or `tests/<file>` with no `::` for a whole file. The two forms are two hands. A single test is the **writer's**: it removes the named test with an `Edit` in its own spec tree, which the lane hook already permits it. A whole file is the **script's**, removed by `scripts/pair.sh red` before it commits, because no agent can remove a file under `tests/` — the lane hook denies every hand the shell that would do it. Whole-file targets are explicitly not the writer's, and it does not attempt them.

An excision line needs no `kills:`, no `bite:` and no `existing:`: its target is its own `existing:` clause, the cited rule is what a reviewer adjudicates it against, and nothing is being pinned. The four-line cap does not apply — a sweep removes what it removes — but every line still names one target and one rule, and "this test is inconvenient" is not a rule number. Section 6 already forbids making a failing test go away, and that is the failure mode this block would otherwise be the tool for.
...
`kills:` names one plausible implementation that is wrong in a way a user would notice and that this line, and only this line, rules out. "Returns the wrong type" and "raises" are not implementations; "loads the preset whose name sorts first instead of the one asked for" is. A line for which you cannot name one is not a behavior and does not go in. `bite:` is measured, never assumed: a threshold you believe sits outside HEAD's range and does not is a line that ships green, and the reviewer cannot catch it because it is barred from the implementation. The measurement is the `bite-prober`'s, never yours: once the behavior lines are drafted, spawn it with the slug, the numbered lines with their inputs, and the surface under test, nothing else, and paste its return into each `bite:` verbatim, value and command. You run no probe yourself. Where the surface does not exist yet there is no measurement to take and an import error proves nothing, so the prober returns the **null stub** the line fails instead — the module present, exports named, every function returning its zero value — and the clause names it. `existing:` carries the grep you ran and its result, not your belief; a line with an existing test does not go in either. A line whose `kills:` is a wording change is copy (`docs/testing.md` rule 9) and does not go in.
