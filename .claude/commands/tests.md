...
**Kind.** The block's first line is `kind: new`, `kind: characterization`, `kind: refactor` or `kind: excision`. It selects two things: the grammar the rest of the block is written in, and what the red run in section 3 expects — red for new behavior, green for the other three. The gauntlet-arbiter reads it as structure and rules on nothing in it beyond the grammar it selects; the writer reads it from the committed file and grades the red run by it; `scripts/pair.sh red` reads it from the same commit and acts on `excision`. Written here, in the draft, so it is approved with the block.
...
- **`GREEN N (expected)` on `kind: characterization`, `refactor` or `excision`** — there is no pre-change state to fail against, and green is the result. Say so in the report instead of passing over it. On an excision the removed tests are gone from the run by construction, so what green describes is the tests that remain in the files the block touched.
