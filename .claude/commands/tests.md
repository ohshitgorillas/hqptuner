---
description: Author tests for new or changed behavior from an approved spec block, through the blind scrivener, always. Argument: what changed (module, behavior, or "the working tree").
---
...
The chain is tests-first: the spec is approved at stage 2 of the plan gate, the tests are written from the spec by the blind `scrivener` in a tree that holds no implementation, the red run in that tree proves they bite (`docs/testing.md` rule 8) and the writer certifies it, and only then does the implementation land beside them. You never write under `tests/`, in any tree, and `hooks/tests-lane.py` denies the attempt; the writer is the only hand on a test file, and it moves only on a committed spec line. Section 6 is invoked from sections 3 and 5 rather than walked through in order.
...
## 0. The tests-only lane

A change confined to `tests/` — a test that violates `docs/testing.md` and has to go, or to be replaced — skips the plan gate and the red run alike. No `detective`, no plan, no `prosecutor`, no owner plan approval, and no bite proof. Four steps:

1. Draft a `motion: strike` block (the test goes) or a `motion: amend` block (the test goes, and one line names the behavior that replaces it), per `docs/testing.md` "Strike motions" and "Amend motions".
2. Send it to a `arbiter`, which resolves each line's quoted assertion against the test file itself — `tests/` is open to it, and these lines rest on nothing else — and writes `gauntlet/specs/approved/<slug>.txt` on `READY`.
3. `${CLAUDE_PLUGIN_ROOT}/scripts/pair.sh open <slug>`, then the `scrivener` removes the targets and writes the replacements its `as:` fields name. A whole-file target is removed by the driver, not the writer.
4. `${CLAUDE_PLUGIN_ROOT}/scripts/pair.sh merge <slug>`, which runs `${CLAUDE_PLUGIN_ROOT}/scripts/strike-diff.py` over the landed diff against the committed block and refuses the land on any target the block named that the diff did not satisfy.

Two commands, `open` and `merge`, and no red run between them. What the lane drops is what polices an implementation phase, and a tests-only change has none. What still holds: I never write under `tests/`, and never decide on my own that a test I find inconvenient pins nothing.

The lane covers a change confined to the test files its own block names. A replacement that also needs a new or edited shared fixture — `tests/conftest.py`, a `tests/fake_*.py`, anything under `tests/support/` — trips `UNNAMED <path>` at the merge, and that change takes the ordinary chain, plan gate and red run included.
...
Read whatever you need of the implementation — that is your job, not the writer's — and distil it into a **spec block**. Seven parts, the first one line, the second the owner's words:

**Kind.** The block's first line is `kind: new`, `kind: characterization`, `kind: refactor`, `motion: strike` or `motion: amend`. It selects two things: the grammar the rest of the block is written in, and what the red run in section 3 expects — red for new behavior, green for characterization and refactor, and no red run at all for the two tests-only motions, which take the lane in section 0 and are checked at the merge instead. The arbiter reads it as structure and rules on nothing in it beyond the grammar it selects; the writer reads it from the committed file and grades the red run by it where there is one; `${CLAUDE_PLUGIN_ROOT}/scripts/pair.sh merge` reads it from the same commit and routes on `strike` and `amend`. Written here, in the draft, so it is approved with the block.

**Brief.** Directly under the `kind:` or `motion:` line, a `brief:` section: the owner's words that asked for this work, verbatim, the revised words where the owner revised them, each line prefixed `> `, nothing authored by you. The prefix keeps the gates off the owner's text: `check_spec_draft.py` reads an item only at `^\s*N. ` and the reviewer separator only at `^spec-reviewer`, and `pair.sh` reads `kind:` and `motion:` only at line start, so a sentence beginning with a digit, "spec-reviewer", "kind:" or "motion:" stays verbatim behind it. Required on `kind: new`; on the other structure lines it carries the owner's words when the owner asked for the work in words, and reads `brief: none` when `/tests` was invoked over existing code with none. The arbiter reads it as structure and cuts a line that contradicts a sentence of it; the writer reads it from the committed file and refuses such a line. It is stage 1's `brief:` block carried down, so an owner revision arriving after `open` is a return to stage 2 and a `respec`.

**`motion: strike` is the one block that removes tests instead of adding them**, and its grammar lives in `docs/testing.md` "Strike motions": strike lines in place of behavior lines, no mixed block, one target and one real rule number per line. Section 6 already forbids making a failing test go away, and that is the failure mode this block would otherwise be the tool for.
...
`kills:` names one plausible implementation that is wrong in a way a user would notice and that this line, and only this line, rules out. "Returns the wrong type" and "raises" are not implementations; "loads the preset whose name sorts first instead of the one asked for" is. A line for which you cannot name one is not a behavior and does not go in. `bite:` is measured, never assumed: a threshold you believe sits outside HEAD's range and does not is a line that ships green, and the reviewer cannot catch it because it is barred from the implementation. The clause is a run's output pasted with its command. Where the measurement is one existing runner invocation, a `pytest` node id, a `node --test` file or a one-line import call, you run it and paste command and output verbatim, as `existing:` already is. Where it needs a throwaway script, a rendered state space or more than one command, spawn the `examiner` with the slug, the numbered lines with their inputs, and the surface under test, nothing else, and paste its return into each `bite:` verbatim, value and command. Where the surface does not exist yet there is no measurement to take and an import error proves nothing, so the prober returns the **null stub** the line fails instead — the module present, exports named, every function returning its zero value — and the clause names it. `existing:` carries the grep you ran and its result, not your belief; a line with an existing test does not go in either. A line whose `kills:` is a wording change is copy (`docs/testing.md` rule 9) and does not go in.
...
### The pre-check runs before the reviewer

Write the draft to `gauntlet/specs/drafts/<slug>.txt` (gitignored, in-tree, free) and run `.venv/bin/python scripts/gates/check_spec_draft.py gauntlet/specs/drafts/<slug>.txt`, free by relative path. It reports the string shapes the reviewer cuts on sight: a second outcome or an absence word in an outcome sentence, a vague word, a missing clause, an `existing:` that names a file and not a test, a `bite:` with no command. Repair every flag you agree with before spawning; a flag you judge false stays, and the reviewer rules on it. The reviewer's letters remain the authority; the pre-check only saves the round a string shape would have cost.

### The arbiter runs before the user sees the spec

Once the draft is clean, spawn the `arbiter` with a `slug:` line, the slug `open` will take, then the `kind:` line, the `brief:` section and the behavior lines, nothing else. **The brief is the bare block.** The reviewer rejects a padded brief on sight with no verdicts, so a padded brief costs you a round every time. What you know about the code goes into `bite:` with its command, or nowhere. What you think is out of scope stays out of the block. What you want to ask the reviewer, you do not ask; you write the line and let the verdict answer. It returns a gate verdict — `READY`, `ANOTHER PASS` or `ESCALATE` — then its `discriminates:` line, then one verdict per line, `KEEP`, `DELTA` or `CUT` with a reason, plus its two stub lines and a surviving count. Apply every `CUT` and fold every `DELTA` into the named existing test before presenting; a verdict you disagree with stays applied and goes into the plan as one line of disagreement for the user to rule on.

**The reviewer holds the gate, and the loop runs until it opens.** On `ANOTHER PASS` you repair the block against the named repair and send it back to the **same reviewer** with `SendMessage`, naming each line whose text changed and supplying your previous round's verdicts for the rest; the reviewer re-runs the changed lines and carries the others. Every finding in the return is answered one of two ways: the named repair, with that line's text changed, or a citation the reviewer lacked, `file:line` or command output, quoted. A reviewer note naming a file you can read is a finding: read the file, return the value. Anything else gets `REJECTED: EVASION`, and that reviewer is finished. One reviewer per spec block, for the life of the block: it holds the rounds it has already ruled on, and a fresh `Agent` call throws that away, re-litigates settled lines, and is the loop that never terminates. Spawning a second reviewer for a block that already has one is a defect on the same footing as skipping the review, whatever the reason: a long wait, a context you would rather start clean. The two exceptions are `REJECTED: STEERING` and `REJECTED: EVASION`: that reviewer printed no verdicts, so there is nothing to carry, and the bare block goes to a fresh `Agent`. Name the abandoned agent in the report to the user; never `SendMessage` a rejecting agent. There is no round limit and no discretion in it: a draft you present early costs the user directly, and the user has ruled that first drafts are not to reach them. Presenting a block the reviewer has not marked `READY` is a defect on the same footing as skipping the review. On `ESCALATE` you stop redrafting and put the design question to the user in the plan — the surface cannot be pinned this way, and the reviewer's alternative goes with it. A verdict you believe is wrong on a fact the reviewer is barred from reading goes to the reviewer first, as that fact with its `file:line`; it reaches the user only if the reviewer holds the finding after it, as the applied verdict plus your one line of disagreement. `READY` is the reviewer's to grant; being seen by the user is not the reviewer's to withhold.
...
- **Rendered geometry is measured, never inferred from source constants.** ViewBox sizes, plot rectangles, which pane carries which attribute, whether a trace spans its plot, whether a state is reachable: the `examiner` renders the whole state space in one throwaway probe and returns every number the block will assert, counts before lists, the probe deleted in the same command that runs it. Sanity-check any count that comes back as all states or zero states. Every measured number becomes a `bite:` field, pasted from the prober's return with its command.
- **A `gauntlet/plans/drafts/` defect bullet may describe an uncommitted tree, not HEAD.** Name the commit or dirty file each bullet was observed on; a mechanism you cannot find in the committed tree is fixed already or lives in someone's uncommitted work, and it earns no `kills:` clause.
...
On stage-2 approval, the arbiter has already written the approved block to `gauntlet/specs/approved/<slug>.txt`, so open the two worktrees this run needs — one action:

```
${CLAUDE_PLUGIN_ROOT}/scripts/pair.sh open <slug>
```

`open` takes the slug alone and derives the spec path from it, `gauntlet/specs/approved/<slug>.txt`, because `hooks/specs-lane.py` lets the arbiter write that folder and lets nothing else write it: the file it reads is one only the arbiter can have put there. The file is the approved block verbatim, then a line reading exactly `--- spec-reviewer READY ---`, then the reviewer's last output beneath it. That output is the reviewer's own: it wrote every round that carried verdicts to `gauntlet/reviews/<slug>.<N>.txt` itself, a rejection round writing nothing, and `open` refuses the spec file unless the section after the separator matches the newest of those files and that file starts with `READY`. `gauntlet/specs/` is gitignored and inside the tree, so writing it there is free; `open` commits it into the spec tree as `gauntlet/specs/approved/<slug>.txt`, the first commit on the branch, before any implementation exists. From then on the block is read from git, by the writer and by any reviewer spawned later, never from a brief you typed.
...
**Every spec goes to the writer, whatever its size.** In a single message: spawn the `scrivener` with two absolute paths inside the spec tree — the committed spec, `.claude/worktrees/<slug>-spec/gauntlet/specs/approved/<slug>.txt`, and its target file, `.claude/worktrees/<slug>-spec/tests/<file>` — those two paths and nothing else, never the block inline; and enter the impl tree and start implementing (section 4). Writer and implementation run concurrently; blindness holds by construction, because the implementation does not exist yet and when it does it is in a tree the writer never opens. A one-line spec spawns the writer like a six-line one: the writer's cost is a spawn that runs beside you, and the alternative is the implementer's hand on the test.

The writer refuses a brief that carries anything beyond those paths and a known-bug list, a spec that is not committed and clean at its tree's HEAD, a delta that names no newer `spec:` commit, and a delta naming a test no `existing:` clause names. A refusal is reported to the user as a refusal, like a reviewer's `ANOTHER PASS`; it is never worked around. A line the writer returns as `UNTESTABLE N` goes back to the same arbiter before the red run: the line is rewritten or cut, the re-approved file landed with `${CLAUDE_PLUGIN_ROOT}/scripts/pair.sh respec <slug>` (one metered action; it commits `spec: <slug>` on the open spec branch and refuses a file whose reviewer section is the round already committed), and the writer sent the delta naming that commit.

**The lanes are enforced, not trusted.** `hooks/tests-lane.py` denies a write under `tests/` by anyone but the writer, and by the writer anywhere but its spec tree's `tests/`, at the write; `pair.sh merge` refuses again if the spec tree wrote outside `tests/` or the impl tree wrote inside it. That rule is what makes the two branches combine without conflict, so treat a lane denial as a misplaced file, never as something to argue with.
...
```
${CLAUDE_PLUGIN_ROOT}/scripts/pair.sh red <slug>
```
...
- **`GREEN N` on `kind: new` — bite failure.** The test passes against a tree that lacks the change, so it constrains nothing. Its `kills:` clause named an implementation the test does not actually distinguish, which means the approved line is wrong, and an approved line is not yours to tighten. Return to stage 2: the corrected block goes to the same arbiter, then to the user for a new approval word, and the approved file is landed with `${CLAUDE_PLUGIN_ROOT}/scripts/pair.sh respec <slug>` before the writer is sent the delta naming that commit and `pair.sh red` run again. Stage 1 instead, with the same prosecutor, if the plan itself is what was wrong. Implementation keeps running in its own tree while you do.
- **`GREEN N (expected)` on `kind: characterization` or `refactor`** — there is no pre-change state to fail against, and green is the result. Say so in the report instead of passing over it. The two tests-only kinds reach no red run at all, so no verdict of any sort is owed for them.

You do not read the saved output yourself; you will see it once, at the merge, inside the test-check brief that goes verbatim to the arbiter.
...
**Green** — the merge has printed a `TEST CHECK <slug>` line or block. The one-line form, `tests identical to the red commit; PIN by construction`, means the landed tests are byte-identical to the red commit, which is `PIN` by the reviewer's own definition: no reviewer round, proceed to `/task-check`. On `motion: strike` and `motion: amend` the merge prints a `MECHANICAL CHECK <slug>` block instead, one verdict line per target, and there is no brief to forward and no reviewer round: a verdict other than `OK` stopped the merge with dev untouched, so a merge that landed is that check having passed. The block form carries the test files, their diff from the red commit, and the saved red output. Forward it verbatim, by `SendMessage`, to the arbiter that printed `READY` for this block — the whole block from `TEST CHECK` to `END TEST CHECK` and not one sentence of yours around it, because any sentence is steering and the reviewer rejects on it. If that reviewer is gone (it rejected a brief for steering or evasion, or the session lost it), spawn a fresh `arbiter` with the same block; it reads the spec and the verdicts from `gauntlet/specs/approved/<slug>.txt` in git and needs nothing from you. It returns `PIN`, `SOFT`, `MISSING` or `EXTRA` per line. All `PIN` — run `/task-check` from the main checkout. **On `motion: strike` the polarity of one verdict inverts**: `MISSING` is the success condition, because the line asked for a test to stop existing, and a line that comes back anything else is a target that survived. `EXTRA` still counts zero. Everywhere else `MISSING` keeps its meaning — dev carrying a line nothing pins — and that is why the inversion is keyed off the committed `motion:` rather than off the reviewer's judgement. It binds the one container and `:8090`, host-wide, which the worktrees do not isolate, so it stays post-merge and stays in the main checkout. Anything else — dev already carries a test that no longer pins its line. Report it to the user before `/task-check`; the repair is a follow-up commit on dev restoring the test from the red commit (`git restore --source <red-commit> -- tests/<file>`, the one shell write onto `tests/` the lane hook passes, because it copies a git object and types nothing), or a return to stage 1 if the spec is what has to move. Never a silent green.

Abandoning the work instead: `${CLAUDE_PLUGIN_ROOT}/scripts/pair.sh abort <slug>` removes both trees and branches. `${CLAUDE_PLUGIN_ROOT}/scripts/pair.sh list` shows the open pairs and is free.
...
- **The spec is wrong.** You mis-stated the behavior, or the plan did. That is not adjudicated here: an approved line is closed, and rewriting it to match what the code does is the failure this chain exists to catch. Return to stage 1 with the same prosecutor, carrying the revision; the new block goes through stage 2, the same arbiter, and a new approval word, and lands as a new `spec: <slug>` commit through `${CLAUDE_PLUGIN_ROOT}/scripts/pair.sh respec <slug>` before the tests are redone. Editing a writer's test to agree is never the path; a writer's test changes only by a delta to the writer, and only once a re-approved spec line calls for it.
