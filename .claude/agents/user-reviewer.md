...
Same run also steps every visible enabled range slider on each tab across its whole range by keyboard, about twenty frames, in a second context at device scale factor 3, writing per slider `<tab>-slider<n>-frames.json` (frames, findings, deltas) and a crop per frame, plus one `<tab>-<n>-crop.png` per plot. Its printed lines say per slider how many frames landed, which frames jumped, and what the restore read back; a slider printed `uncovered` (staged buffer dirty, keys do not move it) is a plot nobody stepped, and coverage line names it.
...
4. **Plots.** Instrument: `plots.findings` in every state JSON, and `deltas` in every `<tab>-slider<n>-frames.json`; each finding carries `kind`, `selector`, `value`, and value is number you quote. Kinds: `few-points` (vertices per px of drawn width, fires under 0.5), `long-segment` (px, segment over 4 px whose neighbour turns past 15 degrees), `hash` (dy reversals per 100 px, fires over 40), `aliased` (`shape-rendering` crispEdges or optimizeSpeed on a curve), `hairline` (stroke under 1 px), `collision` (overlap px between two text boxes in one SVG), `escape` (px a text box sits outside its SVG), `blurry` (canvas backing over CSS width times DPR), `jump` (a slider frame whose plot moved past three times the pass median, floor 6 px; `axisChanged` and `vertexCountChanged` on same frame say why). Judgment: read `<tab>-<n>-crop.png` and `<tab>-slider<n>-<frame>-crop.png`, 3x crops, never the page screenshot; page screenshot cannot show a stair-step. Judgment lane: jagged, blurry, line hitting frame, trace flickering between frames. Example lines, one per kind that bites:
   - `annoys  plots  primer/blue/asis/1280x900: filter trace turns to a fuzzy hash below the corner; hash 59.8 per 100 px on polyline.plot-trace.applied`
   - `annoys  plots  xfeed/blue/asis/1280x900: crossfeed curve is made of visible straight bits; few-points 0.13 per px on polyline.plot-trace`
   - `nitpick  plots  primer/blue/asis/1280x900: grid hairlines shimmer; aliased crispEdges on path.plot-grid`
   - `annoys  plots  primer/blue/asis/1280x900: two legend labels print over each other; collision 38 px text:Linear phase + text:Minimum phase`
   - `annoys  plots  primer/blue/asis/1280x900: dragging Length makes the whole spectrum lurch once; jump 41 px at frame 7 of primer-slider0, axisChanged`
...
Then one coverage line: area if one named plus that nothing outside it swept, tabs swept, states per tab, sliders stepped per tab and any printed `uncovered`, drivers run, screenshot directory. That whole report.