...
**`kind: repair` is the one block that has you remove a test and write its replacement, both halves in one line.** Its line shape is in `docs/testing.md` "Repair blocks": the `excise` target with its `rule:` and quoted `assertion:` is removed exactly as an excision target is, and `replace:` states the behavior the replacement pins while `as:` names the test it must land as. Write that test the way you write any other, from `replace:` and `kills:` alone. The target is always a single test, never a whole file, and a repair line carries no `bite:` — the behavior is already in HEAD, which rule 8 exempts. There is no red run for this kind: your return ends at your own report.

**`kind: excision` is the one block that has you remove tests rather than write them.** Its line shape is in `docs/testing.md` "Excision blocks", and you write no test at all. A single-test target (`tests/<file>::<test>`) is yours: remove exactly that test from that file with an `Edit`, leaving every other test in the file byte-identical, and report the removal per line. A whole-file target is **not yours** — `scripts/pair.sh merge` removes it, because the lane hook denies you and every other agent the shell that would do it. Pass over those lines; do not empty the file by hand as a substitute, and do not report them as done. The `existing:` rule below does not bind an excision line: its target is its own `existing:` clause. A target you cannot find, or a `rule:` that does not fit the quoted assertion, is a finding you report and stop on, exactly like a gap in a spec.
...
On `kind: excision` and `kind: repair` there is no red run and no verdict round: neither has an implementation phase for a bite to be proof about, and the landed diff is checked against the block by `scripts/excision-diff.py` at the merge. Your return ends at your own report.

On every other kind, after you report, the orchestrator commits your tests and runs them with `scripts/pair.sh red`, which saves the output to a file and prints nothing else. It then sends you that path. You read the output and return one verdict per spec line, nothing around them:
