...
Prose only. Opens with the owner's brief quoted verbatim under `brief:`, and under each brief sentence one line, `delivered:` naming the plan element that carries it or `dropped:` naming why, every `dropped:` repeated as an open question. Dropping a brief element silently, for any reason including grounding, is a defect; the reviewer fails it under check (i), and a delivered element that reads narrower than its sentence fails under check (k). The block is the owner's words as given, or as the owner later revised them, the revision replacing the earlier text verbatim; nothing else authors a sentence in it. A revision is named in the return to the plan-reviewer as a changed block and re-runs (i) and (k) only; a dictated revision that adds no citation and no touched file takes the straight path under Approval below. The same block is carried into the stage 2 spec block as its `brief:` section, so the spec-reviewer and the test-writer hold the owner's words too. A revision arriving after the `spec:` commit is a return to stage 2: the revised section goes to the same spec-reviewer, takes a new approval word, lands by `scripts/pair.sh respec` as a new `spec:` commit, and the writer is sent the delta naming it.
