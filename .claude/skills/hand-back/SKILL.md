...
All three are unbriefed. A brief that names a change, a file, a check or an expected result is refused on sight with `REJECTED: STEERING`, and the refusal is reported to the user like a gauntlet-arbiter `ANOTHER PASS`; the bare brief then goes to a fresh agent. Each returns a severity-sorted list under seven fixed categories and no verdict; the list is your input to verify and relay, never a grade to report.
