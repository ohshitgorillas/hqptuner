...
No threshold here comes from ``hqptuner/engine/junkadvisor.py``. The two share reading
...
Every class boundary below sits on this corpus, at the trough or the gap of that statistic's
own distribution, and carries the reading that places it.
...
#: Positives a run needs to be a run. The smallest cliff-only album in the corpus has four rows.
MIN_RUN = 4
