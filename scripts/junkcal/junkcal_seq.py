...
``scripts/junkcal/junkcal_eval.py`` scores one row at a time against that row's own statistics, which is the
...
The corpus is ``manifest.json``, written by ``scripts/junkcal/junkcal_split.py``: one file per album, one
