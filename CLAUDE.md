...
- **Commit without asking; never push without asking.** Committing to the working branch needs no approval word. Push and the promotion chain take the owner's word: `/ship`, `git push` and promotion to `beta` or `main` happen on it alone. `git commit` is metered on the change budget.
...
- **Markdown is written with `Write`/`Edit`, never from the shell.** The soft-wrap and changelog-style gates hang off those tools' `PostToolUse` payload and a shell write (`sed -i`, redirect, heredoc, script) skips both; `.claude/hooks/md-by-tool.py` denies such a command. Reads, `git`, `rm`, `make` and the gate scripts pass. The trivia judge runs once per turn at `Stop` over every markdown line the working tree adds, untracked files included, and holds the turn open once on a flag.
