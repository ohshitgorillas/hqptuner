...
- `make check` = `lint lint-js test test-js` (ruff, black, xenon B/A/A, vulture, strict mypy, import-linter, file-length + test-assertion gates, offline pytest at a 93.9% coverage floor, JS gates incl. eslint, tsc `--checkJs`, knip, jscpd at 2.3%, css-dead). `make test` = offline suite. `make test-live` adds `live`-marked tests (needs reachable hqplayerd). Pre-commit runs same gates except JS test suite, deliberately.
