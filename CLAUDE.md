...
- **Branches:** `dev` (scratch, where work lands) → `beta` (testers, publishes `ghcr.io/…:beta`) → `main` (release, publishes `:latest`). Promotion between them is the user's call, never an agent's — do not merge to `beta` or `main` without being told to. When told to, ship with `/ship dev|beta|main` (`scripts/ship.sh`): it runs the gate, pushes, ff-only merges up the chain, tags `v<version>` on a `main` ship, and bumps the `/srv` submodule pointer — one metered action for the whole promotion, so the change budget does not strand you mid-chain. Author the release commit (version bump in `pyproject.toml` + `hqptuner/__init__.py`, `CHANGELOG.md` heading) before running it; `ship.sh` refuses a dirty tree and never commits or force-pushes.
