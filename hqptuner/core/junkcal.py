...
While the detector's eligibility gate passes, each tick appends one JSON Lines row: the latched verdict, the junk
filter auto-pilot resolves from it, the engine context, the aggregate's coverage and grid, and the windowed minimum
spectrum at or above the lossy window's floor. The spectrum is the metering tap's own, which sees the source rate.
...
def _spectrum(agg: SpectralAggregate) -> list[list[float]] | None:
    """Return the windowed minimum as ``[hz, db]`` pairs at or above the lossy window's floor, or None until earned."""
    window = agg.window_min_db()
    if window is None:
        return None
    floor = junkadvisor.LOSSY_WINDOW_HZ[0]
    pairs = ([junkadvisor.hz(i, agg.bins, agg.bandwidth), db] for i, db in enumerate(window))
    return [pair for pair in pairs if pair[0] >= floor]
...
def _row(mgr: ConnectionManager) -> dict[str, Any] | None:
    ...
    if reader is None or ctx is None or not ctx.playing or agg is None:
        return None
    if not junkadvisor.eligible(agg.seconds, ctx.samplerate, agg.bandwidth, agg.bins, sdm=ctx.sdm):
        return {}
