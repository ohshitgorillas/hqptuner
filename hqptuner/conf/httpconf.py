...
class AuthRefused(httpx.HTTPStatusError):
    ...
    settle loop on this side already catches ``httpx.HTTPError``, and an
    auth refusal is still a wire fault to all of them — so control flow at
    those sites is unchanged and none of them had to learn a new type. What
    changes is that the sites which CAN act on it catch this first, and that
    the message is a sentence rather than httpx's generated one with its link
    to MDN.
    """
