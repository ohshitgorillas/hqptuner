...
``manager.backup_or_cached``) plus the write-path edges around it: a write
...
async def test_a_write_refuses_an_empty_backup_archive(
    ...
    with pytest.raises(presetconf.GroundingError, match="nothing to build a restore"):
        await http_manager.backup_or_cached(for_write=True)
...
async def test_a_write_refuses_the_cached_fallback_too(
    ...
    await http_manager.backup_or_cached()  # cache warmed while healthy
    http_daemon["_empty"] = True
    with pytest.raises(presetconf.GroundingError):
        await http_manager.backup_or_cached(for_write=True)
...
async def test_a_read_falls_back_to_the_cached_archive(
    ...
    healthy = await http_manager.backup_or_cached()
    http_daemon["_empty"] = True
    assert await http_manager.backup_or_cached() == healthy
...
async def test_a_read_with_no_cache_passes_the_archive_through(
    ...
    http_daemon["_empty"] = True
    passed_through = await http_manager.backup_or_cached()
...
async def test_a_healthy_backup_is_cached_for_the_outage(http_manager: ConnectionManager) -> None:
    await http_manager.backup_or_cached()
    assert http_manager.last_healthy_backup is not None
...
async def test_apply_with_switch_to_reports_the_loaded_preset(http_manager: ConnectionManager) -> None:
    await http_manager.save_preset("Base")
    report = await http_manager.apply({}, {}, switch_to="Base")
...
async def test_apply_with_switch_to_restores_the_presets_config(
    ...
    await http_manager.save_preset("Base")
    await http_manager.apply({}, {"title": "Drift"})
    await http_manager.apply({}, {}, switch_to="Base")
...
async def test_apply_with_switch_to_lands_staged_edits_on_top_of_the_preset(
    ...
    await http_manager.save_preset("Base")
    await http_manager.apply({}, {"title": "Tweaked"}, switch_to="Base")
...
async def test_apply_engine_without_credentials_reports_not_submitted() -> None:
    ...
    result = await manager.apply_engine({"cuda": "0"})
...
def test_persist_backup_writes_the_archive_to_disk(tmp_path: Path) -> None:
    ...
    path = manager.persist_backup(b"archive-bytes")
...
def test_persist_backup_tolerates_an_unwritable_directory(tmp_path: Path) -> None:
    ...
    assert manager.persist_backup(b"archive-bytes") is None
