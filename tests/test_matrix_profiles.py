...
async def test_save_adds_the_profile_to_the_daemon_list(
    ...
    await http_manager.matrix_profile_action("save", "Crossfeed EQ")
...
async def test_delete_removes_the_profile_from_the_daemon_list(
    ...
    await http_manager.matrix_profile_action("delete", "Mch-to-Stereo mixdown")
    assert "Mch-to-Stereo mixdown" not in http_daemon["_matrix_profiles"]


async def test_load_sets_the_active_profile_label(http_manager: ConnectionManager, http_daemon: dict[str, Any]) -> None:
    await http_manager.matrix_profile_action("load", "Default")
    assert http_daemon["matrix_active"] == "Default"


async def test_profile_post_carries_the_complete_form(
    http_manager: ConnectionManager, http_daemon: dict[str, Any]
) -> None:
    await http_manager.matrix_profile_action("save", "New")
...
async def test_load_preserves_the_crossfeed_enable(
    http_manager: ConnectionManager, http_daemon: dict[str, Any]
) -> None:
...
async def test_load_preserves_post_process_values(http_manager: ConnectionManager, http_daemon: dict[str, Any]) -> None:
    http_daemon["post_bauer_frequency"] = "900"
    await http_manager.matrix_profile_action("load", "Default")
    assert http_daemon["post_bauer_frequency"] == "900"


async def test_save_does_not_reload_the_engine_twice(
    http_manager: ConnectionManager, http_daemon: dict[str, Any]
) -> None:
...
async def test_unknown_action_is_refused(http_manager: ConnectionManager) -> None:
    with pytest.raises(ValueError, match="unknown matrix profile action"):
        await http_manager.matrix_profile_action("rename", "X")
