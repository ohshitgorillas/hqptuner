...
async def test_a_rate_the_engine_offers_resolves_to_its_list_index(live_manager: LiveManager) -> None:
    ...
    # is on the rate in Hz, and `SetRate` wants the index (protocol.md §6). The
    # join is on the TIER: 352800 resolves to its 48k member 384000, index 4.
    manager, _, _ = await live_manager()
    assert chain.rate_index_for(manager, "352800") == "4"
