...
import pytest
from conftest import CommandLog, DaemonFactory

from hqptuner.config import Config
from hqptuner.lanes import livelane, livemap
from hqptuner.manager import ConnectionManager

#: A manager already connected to a fake daemon, plus that daemon's traffic.
LiveManager = Callable[..., Awaitable[tuple[ConnectionManager, CommandLog]]]
...
async def live_manager(daemon: DaemonFactory) -> AsyncIterator[LiveManager]:
    ...
    engine already pinned, `_deaf="SetMode"` starts one whose `SetMode` answers
    OK without applying (protocol.md §4: OK is not proof).
    """
    started: list[tuple[ConnectionManager, asyncio.Task[None]]] = []

    async def build(**overrides: str) -> tuple[ConnectionManager, CommandLog]:
        port, log = await daemon(**overrides)
...
async def test_a_verified_rate_write_is_remembered_under_its_family(live_manager: LiveManager) -> None:
    ...
    assert manager.live_rates["pcm"] == "352800"
...
async def test_a_rate_write_that_never_verified_is_not_remembered(live_manager: LiveManager) -> None:
    ...
    manager, _ = await live_manager(_deaf="SetRate")
    await livelane.apply_now(manager, {"rate": "352800"})
    assert "pcm" not in manager.live_rates
...
async def test_pinning_auto_forgets_the_remembered_pcm_rate(live_manager: LiveManager) -> None:
    ...
    manager, _ = await live_manager()
    await livelane.apply_now(manager, {"rate": "44100"})
...
async def test_pinning_auto_forgets_the_remembered_sdm_rate(live_manager: LiveManager) -> None:
    ...
    await livelane.apply_now(manager, {"rate": "0"})
    assert "sdm" not in manager.live_rates
...
async def test_a_dormant_44k_base_pcm_pin_is_reported_as_its_48k_tier(live_manager: LiveManager) -> None:
    manager, _ = await live_manager()
    await livelane.apply_now(manager, {"rate": "352800"})  # 8x on a 44.1k base
    await livelane.apply_now(manager, {"mode": "sdm"})
    assert livemap.live_overrides(manager)["defaults_samplerate"] == "384000"


async def test_a_dormant_44k_base_sdm_pin_is_reported_as_its_48k_tier(live_manager: LiveManager) -> None:
    manager, _ = await live_manager()
    await livelane.apply_now(manager, {"mode": "sdm"})
    await livelane.apply_now(manager, {"rate": "2822400"})  # DSD64 on a 44.1k base
    await livelane.apply_now(manager, {"mode": "pcm"})
    assert livemap.live_overrides(manager)["defaults_bitrate"] == "3072000"
...
async def test_a_dormant_48k_base_pcm_pin_is_reported_unchanged(live_manager: LiveManager) -> None:
    ...
    manager, _ = await live_manager()
    await livelane.apply_now(manager, {"rate": "384000"})  # 8x on a 48k base
    await livelane.apply_now(manager, {"mode": "sdm"})
    assert livemap.live_overrides(manager)["defaults_samplerate"] == "384000"
...
async def test_a_dormant_48k_base_sdm_pin_is_reported_unchanged(live_manager: LiveManager) -> None:
    manager, _ = await live_manager()
    await livelane.apply_now(manager, {"mode": "sdm"})
    await livelane.apply_now(manager, {"rate": "12288000"})  # DSD256 on a 48k base
    await livelane.apply_now(manager, {"mode": "pcm"})
    assert livemap.live_overrides(manager)["defaults_bitrate"] == "12288000"
...
async def test_the_engines_own_pin_outranks_the_remembered_one(live_manager: LiveManager) -> None:
    ...
    manager, _ = await live_manager(rate="2")
    manager.live_rates["pcm"] = "44100"
    assert livemap.live_overrides(manager)["defaults_samplerate"] == "384000"
...
async def test_a_family_with_no_pin_reports_no_rate_limit(live_manager: LiveManager, field: str) -> None:
    ...
    manager, _ = await live_manager()
    assert field not in livemap.live_overrides(manager)
...
async def test_entering_pcm_re_asserts_the_remembered_pcm_pin(live_manager: LiveManager) -> None:
    ...
    manager, log = await live_manager()
    await livelane.apply_now(manager, {"rate": "44100"})
    await livelane.apply_now(manager, {"mode": "sdm"})
    log.clear()
    await livelane.apply_now(manager, {"mode": "pcm"})
...
async def test_the_re_asserted_pin_is_reported_as_a_rate_setting(live_manager: LiveManager) -> None:
    ...
    await livelane.apply_now(manager, {"rate": "44100"})
    await livelane.apply_now(manager, {"mode": "sdm"})
    report = await livelane.apply_now(manager, {"mode": "pcm"})
...
async def test_a_remembered_rate_the_entered_mode_lacks_is_never_pinned(live_manager: LiveManager) -> None:
    ...
    manager, log = await live_manager()
    await livelane.apply_now(manager, {"mode": "sdm"})
    manager.live_rates["pcm"] = "88200"  # no PCM list this engine offers carries it
    log.clear()
    await livelane.apply_now(manager, {"mode": "pcm"})
...
async def test_a_mode_write_that_never_verified_re_asserts_nothing(live_manager: LiveManager) -> None:
    ...
    manager, log = await live_manager(_deaf="SetMode")
    await livelane.apply_now(manager, {"rate": "44100"})
    log.clear()
    await livelane.apply_now(manager, {"mode": "sdm"})
    assert _rate_writes(log) == []
...
async def test_a_mode_write_that_never_verified_keeps_the_pin_it_left(live_manager: LiveManager) -> None:
    ...
    manager, _ = await live_manager(_deaf="SetMode")
    await livelane.apply_now(manager, {"rate": "44100"})
    await livelane.apply_now(manager, {"mode": "sdm"})
    assert manager.live_rates["pcm"] == "44100"
