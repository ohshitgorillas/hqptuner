...
/**
 * An evaluate result whose changes carry a replace, so `fit` is written.
 *
 * @typedef {EvalOut & { fit: NonNullable<EvalOut["fit"]> }} EvalWithFit
 */

/**
 * A survivor of a refined search over a replace space: `fit`, `score` and
 * `refined` are all written.
 *
 * @typedef {SurvivorOut & Required<Pick<SurvivorOut, "fit" | "score" | "refined">>} TopOut
 */

/** @typedef {{ considered: number, top: TopOut[] }} ScalarResult */
/** @typedef {{ front: TopOut[] }} ParetoResult */
/** @typedef {{ rejected_top: { score: number, changes: ChangeSet, reasons: Failure[] }[] }} RejectResult */
/** @typedef {{ seed: TopOut, best: TopOut }} RefineResult */
...
const EV = /** @type {EvalWithFit} */ (
  evaluateJob({ changes: { replace: [{ remove: [1000], with: [{ type: "peak", f: 1500, q: 1, g: 2 }] }] } }, CTX)
);
...
test("test_fit_range_on_a_replace_spec_is_echoed_back_as_range", () => {
  const res = /** @type {EvalWithFit} */ (
    evaluateJob({ changes: { replace: [{ remove: [1000], with: [], fit_range: [500, 2000] }] } }, CTX)
  );
...
const SRES = /** @type {ScalarResult} */ (
  searchJob({ space: RSPACE, constraints: [], objective: "maximize spot", top: 5 }, SCTX)
);
...
test("test_rejected_top_entries_carry_no_fit", () => {
  ...
  const rej = /** @type {RejectResult} */ (
    searchJob({ space: RSPACE, constraints: [{ metric: "spot", max: 1 }], objective: "maximize spot", top: 5 }, SCTX)
  );
  assert.deepEqual(
...
const RRES = /** @type {ScalarResult} */ (
  searchJob({ space: QSPACE, constraints: [], objective: "minimize sq", top: 5, refine: true }, QCTX)
);
 ...
 * @returns {ReturnType<typeof guidanceFlags>}
 */
