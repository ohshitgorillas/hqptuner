...
//
// RULE 8 EXEMPTION, stated rather than assumed. Six of the nine cases are
// CHARACTERIZATION: they pin the conditions under which nothing is hidden — one
// unlimited filter anywhere in the preset's reach, no ratio class stated at all,
// the 48 kHz family available, a PCM lane, an Auto lane, and a backend that
// pins no base. Each of those is green before the gate exists as well as after,
// and cannot bite on its own. They are here because the gate's whole risk is
// over-hiding, and a hide rule with no fence around it is the defect this file
// exists to catch. The three cases that bite are the three hides.
//
...
const { sdmSubject, sdmNames, classedAs } = await import("../support/easytable.js");
...
test("test_a_preset_reaching_one_unlimited_filter_is_still_offered_in_sdm_at_the_441_base", async () => {
  const subject = sdmSubject(2);
  const names = sdmNames(subject);
  await resetTab({
    mode: "sdm",
    ...PINNED,
    ratios: { ...classedAs(names, "2x"), [names[0]]: "any" },
  });
