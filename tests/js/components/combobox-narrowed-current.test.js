...
async function narrowedField({ favOnly = true } = {}) {
  ...
  // The 1x stage narrows to apodizing filters by default and the quality facet
  // to 3/5; opening both leaves favorites-only as the ONLY facet in play, so
  // `favOnly` alone decides whether the current selection passes and each case
  // says what it means.
  nApod1x.value = "all";
  nQuality.value = 0;
