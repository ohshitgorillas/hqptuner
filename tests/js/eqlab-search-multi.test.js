...
import { FS, band, curve } from "./eqlab-helpers.js";
...
test("test_a_single_append_spec_yields_one_entry_append_changes", () => {
  const res = searchJob(jobOf({ append: { type: "peak", f: 2000, q: 1, g: [1, 2] } }), CTX);
...
test("test_a_target_relative_objective_produces_finite_scores", async () => {
  ...
  assert.equal(Number.isFinite(searchJob(job, ctx).top[0].score), true);
});
